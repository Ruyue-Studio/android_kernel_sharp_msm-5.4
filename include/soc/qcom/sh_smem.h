...
typedef struct 
{
    ...
    unsigned char       fast_disp_info[5];           /* Boot Display Info */
    ...
    unsigned short      sh_als_scale_a;              /* Scale Factor alpha */
    unsigned short      sh_als_scale_g;              /* Scale Factor gamma */
    uint32_t            sh_hw_model;                 /* Model identification number */
    unsigned short      sh_rgb_scale_a[4];           /* Scale Factor alpha(mantissa) */
    unsigned short      sh_rgb_scale_g[4];           /* Scale Factor gamma(exponent part) */
    unsigned char       sh_camCalibData[4096];       /* Three Camera adjustment Data */
    unsigned char       sh_diagenc_gk_ex[16];        /* shdiag edgk */
    unsigned short      shdiag_proxadjex[2];         /* Proximity sensor adjust (for VSYNC-ASYNC) */
    unsigned short      sh_als_sf_alpha[3];          /* Scale Factor alpha ALS/Clear/IR */
    unsigned short      sh_als_sf_gamma[3];          /* Scale Factor gamma ALS/Clear/IR */
