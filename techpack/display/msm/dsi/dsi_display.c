		...
		void *display, u32 bl_lvl)
{
	...
	if ((priv != NULL) && (priv->upper_unit_is_connected == DRM_UPPER_UNIT_IS_NOT_CONNECTED)) {
		DSI_DEBUG("%s: upper unit is not connected\n", __func__);
		return 0;
	}
#endif /* CONFIG_SHARP_DISPLAY */
				  ...
				  loff_t *ppos)
{
	...
	char *buf;
	int rc = 0;
#ifndef CONFIG_SHARP_DISPLAY /* CUST_ID_00015 */
				 ...
				 const struct mipi_dsi_msg *msg)
{
	...
	if (display->ctrl_count > 1 && !(msg->flags & MIPI_DSI_MSG_UNICAST)) {
		...
		if (msg->flags & MIPI_DSI_MSG_LASTCOMMAND) {
			cmd_flags |= DSI_CTRL_CMD_LAST_COMMAND;
		}
#endif /* CONFIG_SHARP_DISPLAY */
		...
		void *data)
{
...
#endif /* CONFIG_SHARP_DISPLAY */
#ifdef CONFIG_SHARP_DISPLAY /* CUST_ID_00012 */
	rc = drm_notify_create_sysfs(display);
	if (rc) {
		DSI_ERR("%s: failed to drm_notify_create_sysfs, rc=%d\n",
								__func__, rc);
		rc = 0;
	}
#endif /* CONFIG_SHARP_DISPLAY */
	...
	if (rc) {
		DSI_ERR("%s: failed to drm_det_init, rc=%d\n", __func__, rc);
		rc = 0;
	}
#endif /* CONFIG_SHARP_DISPLAY */
...
#ifdef CONFIG_SHARP_DISPLAY /* CUST_ID_00012 */
	drm_notify_remove_sysfs(display);
#endif /* CONFIG_SHARP_DISPLAY */
	...
	*out_mode = NULL;
	...
	for (i = 0; i < count; i++) {
		...
		if (cmp->timing.v_active == m->timing.v_active &&
			...
			cmp->timing.h_skew == m->timing.h_skew &&
#endif /* CONFIG_SHARP_DISPLAY */
			 ...
			 u32 flags)
{
	...
	if (timing.h_skew)
		DSI_INFO("mdp_transfer_time=%d, hactive=%d, vactive=%d, fps=%d\n",
			...
			timing.h_active, timing.v_active, timing.h_skew);
	else
#endif /* CONFIG_SHARP_DISPLAY */
	DSI_INFO("mdp_transfer_time=%d, hactive=%d, vactive=%d, fps=%d\n",
			adj_mode.priv_info->mdp_transfer_time_us,
...
#endif /* CONFIG_SHARP_DISPLAY */
#ifdef CONFIG_SHARP_DISPLAY /* CUST_ID_00016 */
		display->panel->mfr = MFR_INIT_VALUE_PANEL_ON;
#endif /* CONFIG_SHARP_DISPLAY */
#ifdef CONFIG_SHARP_DISPLAY /* CUST_ID_00036 */
		display->panel->bias_register = drm_bias_get_otp_voltage();
		drm_bias_enable(true);
#endif /* CONFIG_SHARP_DISPLAY */
#ifdef CONFIG_SHARP_DISPLAY /* CUST_ID_00042 */
		drm_voltage_control_enable(true);
#endif /* CONFIG_SHARP_DISPLAY */
#ifdef CONFIG_SHARP_DISPLAY /* CUST_ID_00049 */
...
#ifdef CONFIG_SHARP_DISPLAY /* CUST_ID_00053 */
		mutex_lock(&display->display_lock);
		rc = drm_gmm_ctrl_gamma_table(display->panel);
		if (rc) {
			DSI_ERR("[%s] failed to ctrl gamma table, rc=%d\n",
			       display->panel->name, rc);
		}
		mutex_unlock(&display->display_lock);
#endif /* CONFIG_SHARP_DISPLAY */
...
int dsi_display_post_enable(struct dsi_display *display)
{
...
#ifdef CONFIG_SHARP_DISPLAY /* CUST_ID_00040 */
		mode = display->panel->cur_mode;
		if ((mode) && (!(mode->dsi_mode_flags & DSI_MODE_FLAG_DMS))) {
	...
	if (!display->panel->panel_mode_switch_enabled &&
			...
			(DSI_MODE_FLAG_DMS | DSI_MODE_FLAG_POMS))) {
		if (display->config.panel_mode == DSI_OP_CMD_MODE) {
			DSI_DEBUG("%s: disponchk in cmd mode\n", __func__);
			rc = drm_det_disponchk(display);
			if (rc) {
				DSI_ERR("%s: failed to drm_det_disponchk, rc=%d\n", __func__, rc);
				rc = 0;
			}
		}
		rc = drm_det_chk_panel_on();
		if (rc) {
			DSI_ERR("%s: failed to drm_det_chk_panel_on, rc=%d\n", __func__, rc);
			rc = 0;
		}
	}
#endif /* CONFIG_SHARP_DISPLAY */
#ifdef CONFIG_SHARP_DISPLAY /* CUST_ID_00012 */ /* CUST_ID_00015 */ /* CUST_ID_00025 */
	if (!(display->panel->cur_mode->dsi_mode_flags &
...
#ifdef CONFIG_SHARP_SHTERM
		{
			...
			DSI_DEBUG("%s[%d] LCDPOW on.", __func__, __LINE__);
			if (shterm_ret != SHTERM_SUCCESS) {
				DSI_ERR("%s[%d] shterm_k_set_info() error.",
							__func__, __LINE__);
			}
		}
#endif /* CONFIG_SHARP_SHTERM */
...
int dsi_display_pre_disable(struct dsi_display *display)
{
	...
	if ((!display->panel->panel_mode_switch_enabled) &&
		...
		if (rc) {
			DSI_ERR("%s: failed to drm_det_pre_panel_off, rc=%d\n",
							__func__, rc);
			rc = 0;
		}
	}
...
exit:
	...
	if (!display->poms_pending) {
		...
		DSI_DEBUG("%s[%d] LCDPOW off.", __func__, __LINE__);
		if (shterm_ret != SHTERM_SUCCESS) {
			DSI_ERR("%s[%d] shterm_k_set_info() error.",
							__func__, __LINE__);
		}
...
#ifdef CONFIG_SHARP_DISPLAY /* CUST_ID_00016 */
int dsi_display_set_mfr(struct dsi_display *display, int mfr)
{
	int rc = 0;
	...
	rc = dsi_panel_set_mfr(display->panel, mfr);
...
int dsi_display_set_drive_mode(void *display, enum msm_drive_mode mode)
{
	struct dsi_display *dsi_display = (struct dsi_display *)display;
	int rc = 0;
	...
	rc = dsi_panel_set_drive_mode(dsi_display->panel, mode);

	DSI_DEBUG("Set drive mode  %d %s", mode,
			rc ? "failed" : "successful");
