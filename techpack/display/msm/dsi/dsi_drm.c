				...
				struct dsi_display_mode *dsi_mode)
{
...
#ifdef CONFIG_SHARP_DISPLAY /* CUST_ID_00040 */
	if (dsi_mode->timing.h_skew) {
		dsi_mode->timing.refresh_rate = 120;
	} else {
	...
	if (dsi_mode->priv_info) {
		...
		dsi_mode->timing.mipiclkhalf = dsi_mode->priv_info->mipiclkhalf;
#endif /* CONFIG_SHARP_DISPLAY */
	}
...
#ifdef CONFIG_SHARP_DISPLAY /* CUST_ID_00040 */
	if (dsi_mode->timing.h_skew) {
		drm_mode->vrefresh = dsi_mode->timing.h_skew;
	} else {
...
#ifndef CONFIG_SHARP_DISPLAY /* CUST_ID_00055 */
		...
		struct msm_display_kickoff_params *params)
{
	...
	rc = dsi_display_pre_kickoff(connector, display, params);
	if (c_conn->connector_type == DRM_MODE_CONNECTOR_DSI) {
		drm_oneshot_hz_recovery(false);
	}
	return rc;
