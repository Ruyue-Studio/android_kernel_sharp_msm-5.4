...
#endif /* CONFIG_SHARP_DISPLAY */
#ifdef CONFIG_SHARP_DISPLAY /* CUST_ID_00045 */
#include "sde_connector.h"
#endif /* CONFIG_SHARP_DISPLAY */
#ifdef CONFIG_SHARP_DISPLAY /* CUST_ID_00048 */
#include "../sharp/drm_oneshot.h"
#endif /* CONFIG_SHARP_DISPLAY */
#ifdef CONFIG_SHARP_DISPLAY /* CUST_ID_00049 */
#include "../sharp/drm_peak_brightness_ctrl.h"
#endif /* CONFIG_SHARP_DISPLAY */
#ifdef CONFIG_SHARP_DISPLAY /* CUST_ID_00053 */
#include "../sharp/drm_gmm.h"
#endif /* CONFIG_SHARP_DISPLAY */
...
#ifdef CONFIG_SHARP_DISPLAY /* CUST_ID_00033 */
struct drm_drive_ctx {
	struct workqueue_struct *delayedwkq;
	struct delayed_work delayedwk;
	struct dsi_panel *panel;
};
static struct drm_drive_ctx drive_ctx = {0};
int drivemode_delayed_ms_other = 0;
int drivemode_delayed_ms_enter_mode3 = 500;
int drivemode_delayed_ms_exit_mode3 = 500;
#ifdef CONFIG_DEBUG_FS
module_param(drivemode_delayed_ms_other, uint, 0600);
module_param(drivemode_delayed_ms_enter_mode3, uint, 0600);
module_param(drivemode_delayed_ms_exit_mode3, uint, 0600);
...
#endif /* CONFIG_SHARP_DISPLAY */
#ifdef CONFIG_SHARP_DISPLAY /* CUST_ID_00033 */
static int dsi_panel_set_drive_mode_sub(struct dsi_panel *panel,
		enum msm_drive_mode mode);
static void dsi_panel_drive_mode_work(struct work_struct *work);
#endif /* CONFIG_SHARP_DISPLAY */
#ifdef CONFIG_SHARP_DISPLAY /* CUST_ID_00031 */
static int dsi_panel_bl_boost(struct dsi_panel *panel, u32 onoff);
static int dsi_panel_boost_work_init(struct dsi_panel *panel, bool Initialize);
static int dsi_panel_create_sysfs(struct dsi_panel *panel);
static void dsi_panel_remove_sysfs(struct dsi_panel *panel);

static unsigned int cancel_delay_ms[DSI_BOOST_MODE_MAX] = {10 * 1000};
module_param_array(cancel_delay_ms, uint, NULL, 0660);
static unsigned int boost_brightness[DSI_BOOST_MODE_MAX] = {BKL_CURR_MAX_BRIGHTNESS};
module_param_array(boost_brightness, uint, NULL, 0660);
#endif /* CONFIG_SHARP_DISPLAY */
#ifdef CONFIG_SHARP_DISPLAY /* CUST_ID_00036 */
static int dsi_panel_parse_therm(struct dsi_panel *panel);
#endif /* CONFIG_SHARP_DISPLAY */
...
#ifdef CONFIG_SHARP_DISPLAY /* CUST_ID_00002 */
static int dsi_panel_set_display_brightness(struct dsi_panel *panel,
	u32 bl_lvl)
{
	int err = 0;
	struct dsi_display *pdisp = NULL;
	unsigned char page_payload[] = {0xF0, 0x55, 0xAA, 0x52, 0x08, 0x00};
	unsigned char payload[] = { MIPI_DCS_SET_DISPLAY_BRIGHTNESS,
					  bl_lvl >> 8, bl_lvl & 0xff};
	struct dsi_cmd_desc brightness_cmd[] = {
		{{0, MIPI_DSI_GENERIC_LONG_WRITE, MIPI_DSI_MSG_USE_LPM, 0, 0,
			sizeof(page_payload), page_payload,
			0, NULL}, 1, 0},
		{{0, MIPI_DSI_GENERIC_LONG_WRITE, MIPI_DSI_MSG_USE_LPM, 0, 0,
	...
	err = drm_cmn_dsi_cmds_transfer(pdisp, brightness_cmd,
...
#ifdef CONFIG_SHARP_DISPLAY /* CUST_ID_00002 */
	rc = dsi_panel_set_display_brightness(panel, bl_lvl);
...
#ifdef CONFIG_SHARP_DISPLAY /* CUST_ID_00036 */ /* CUST_ID_00052 */
	bl = &panel->bl_config;
	if (bl) {
		bl_before = bl->nonlinear_brightness;
		bl->nonlinear_brightness = bl_lvl;
		if (exec) {
			if (bl_before != bl_lvl) {
				drm_bias_set_duty_gmm(panel);
				drm_bias_best_bias_setting(panel);
			}
		}
	}
#endif /* CONFIG_SHARP_DISPLAY */
...
#ifdef CONFIG_SHARP_DISPLAY /* CUST_ID_00031 */
static int dsi_panel_update_dev_brightness(struct dsi_panel *panel, u32 bl_lvl)
{
	struct dsi_backlight_config *bl = NULL;
	int new_dev_brightness = 0;
	struct device *dev = NULL;

	if (!panel) {
		DSI_ERR("%s: Invalid dsi_panel data\n", __func__);
		return -EINVAL;
	}
	bl = &panel->bl_config;
	if (!bl) {
		DSI_ERR("%s: Invalid bkl_config data\n", __func__);
		return -EINVAL;
	}
	new_dev_brightness = dsi_panel_brightness_to_bl(bl_lvl, &panel->linear_params);
	if (bl->dev_brightness == new_dev_brightness) {
		return 0;
	}
	bl->dev_brightness = new_dev_brightness;

	if (!panel->parent) {
		DSI_ERR("%s: Invalid device\n", __func__);
		return -EINVAL;
	}
	dev = panel->parent;
	if (dev) {
		sysfs_notify(&dev->kobj, "backlight", "dev_brightness");
	}
	return 0;
}

static int dsi_panel_update_curr_boosted(struct dsi_panel *panel, bool boosted)
{
	struct dsi_backlight_config *bl = NULL;
	struct device *dev = NULL;
	...
	if (bl->curr_boosted == boosted) {
		return 0;
	}
	bl->curr_boosted = boosted;
	...
	dev = panel->parent;
	if (dev) {
		sysfs_notify(&dev->kobj, "backlight", "curr_boost");
	}
	return 0;
}
#endif /* CONFIG_SHARP_DISPLAY */
...
#ifdef CONFIG_SHARP_DISPLAY /* CUST_ID_00031 */ /* CUST_ID_00036 */
static int dsi_panel_update_backlight_boost(struct dsi_panel *panel, u32 bl_lvl)
{
	...
	if ((bkl_on != prev_state) && bl->curr_boosted) {
		...
		dsi_panel_update_curr_boosted(panel, false);
	...
	if (!bl->curr_boosted) {
		...
		case DSI_BACKLIGHT_DCS:
			dsi_panel_update_dev_brightness(panel, bl_lvl);
			rc = dsi_panel_update_backlight(panel, bl_lvl, true);
			break;
		default:
...
#ifdef CONFIG_SHARP_DISPLAY /* CUST_ID_00052 */
static int dsi_panel_update_backlight_drive_mode(struct dsi_panel *panel)
{
	int rc = 0;
	u32 bl_lvl = 0;
	struct dsi_backlight_config *bl = NULL;
	...
	if (!bl->curr_boosted) {
		...
		case DSI_BACKLIGHT_DCS:
			dsi_panel_update_dev_brightness(panel, bl_lvl);
			rc = dsi_panel_update_backlight(panel, bl_lvl, false);
			break;
		default:
			DSI_ERR("Backlight type(%d) not supported\n", bl->type);
			rc = -ENOTSUPP;
...
#ifdef CONFIG_SHARP_DISPLAY /* CUST_ID_00031 */
	dsi_panel_boost_work_init(panel, true);
#endif /* CONFIG_SHARP_DISPLAY */
...
#ifdef CONFIG_SHARP_DISPLAY /* CUST_ID_00031 */
	dsi_panel_boost_work_init(panel, false);
#endif /* CONFIG_SHARP_DISPLAY */
				  ...
				  struct dsi_parser_utils *utils)
{
	...
	display_mode->priv_info->mipiclkhalf =
			utils->read_bool(utils->data, "sharp,mdss-dsi-panel-mipiclkhalf");
#endif /* CONFIG_SHARP_DISPLAY */
...
#ifdef CONFIG_SHARP_DISPLAY /* CUST_ID_00016 */
	"sharp,mdss-dsi-mfr-60hz-command",
	"sharp,mdss-dsi-mfr-30hz-command",
	"sharp,mdss-dsi-mfr-1hz-command",
#endif /*  CONFIG_SHARP_DISPLAY */
#ifdef CONFIG_SHARP_DISPLAY /* CUST_ID_00033 */
	"sharp,mdss-dsi-drive-mode-normal",
	"sharp,mdss-dsi-drive-mode-game",
	"sharp,mdss-dsi-drive-mode-super_bright",
	"sharp,mdss-dsi-drive-mode-gradation-12bit",
	"sharp,mdss-dsi-drive-mode-high-brightness",
#endif /* CONFIG_SHARP_DISPLAY */
#ifdef CONFIG_SHARP_DISPLAY /* CUST_ID_00037 */
	"sharp,mdss-dsi-on-command-evt",
	"sharp,mdss-dsi-on-command-dvt",
#endif /*  CONFIG_SHARP_DISPLAY */
};
...
#ifdef CONFIG_SHARP_DISPLAY /* CUST_ID_00016 */
	"sharp,mdss-dsi-mfr-60hz-command-state",
	"sharp,mdss-dsi-mfr-30hz-command-state",
	"sharp,mdss-dsi-mfr-1hz-command-state",
#endif /*  CONFIG_SHARP_DISPLAY */
#ifdef CONFIG_SHARP_DISPLAY /* CUST_ID_00033 */
	"sharp,mdss-dsi-drive-mode-normal-state",
	"sharp,mdss-dsi-drive-mode-game-state",
	"sharp,mdss-dsi-drive-mode-super_bright-state",
	"sharp,mdss-dsi-drive-mode-gradation-12bit-state",
	"sharp,mdss-dsi-drive-mode-high-brightness-state",
#endif /* CONFIG_SHARP_DISPLAY */
#ifdef CONFIG_SHARP_DISPLAY /* CUST_ID_00037 */
	"sharp,mdss-dsi-on-command-evt-state",
	"sharp,mdss-dsi-on-command-dvt-state",
#endif /*  CONFIG_SHARP_DISPLAY */
};
...
#ifdef CONFIG_SHARP_DISPLAY /* CUST_ID_00022 */ /* CUST_ID_00041 */
	DSI_DEBUG("sh_hw_handset=%d,sh_hw_revision=%d\n",
		drm_cmn_get_hw_handset(), drm_cmn_get_hw_revision());
	if (drm_cmn_get_hw_handset() == 0 || drm_cmn_get_hw_revision() < 3) {
		reset_gpio_name = "sharp,platform-reset-gpio-evt";
	}
#endif /* CONFIG_SHARP_DISPLAY */
...
static int dsi_panel_parse_bl_config(struct dsi_panel *panel)
{
	...
	rc = utils->read_u32(utils->data,
			"sharp,mdss-dsi-bl-default-level", &val);
	if (rc) {
				...
				bool trusted_vm_env)
{
	...
	if (rc)
		DSI_ERR("failed to linear params, rc=%d\n", rc);
#endif /* CONFIG_SHARP_DISPLAY */

#ifdef CONFIG_SHARP_DISPLAY /* CUST_ID_00036 */
	rc = dsi_panel_parse_therm(panel);
	if (rc)
		DSI_ERR("failed to therm params, rc=%d\n", rc);
#endif /* CONFIG_SHARP_DISPLAY */
...
#ifdef CONFIG_SHARP_DISPLAY /* CUST_ID_00033 */
	panel->drive_mode = MSM_DISPLAY_DRIVE_MODE_NORMAL;
#endif /* CONFIG_SHARP_DISPLAY */
#ifdef CONFIG_SHARP_DISPLAY /* CUST_ID_00031 */
	rc = dsi_panel_create_sysfs(panel);
	if (rc) {
		DSI_ERR("[%s] failed to request create sysfs, rc=%d\n", panel->name,
		       rc);
		goto error_bl_register;
	}
#endif /* CONFIG_SHARP_DISPLAY */
...
#ifdef CONFIG_SHARP_DISPLAY /* CUST_ID_00024 */ /* CUST_ID_00040 */
	int num_mipiclkhalf_modes = 0;
	int loop = 0;
	bool mipiclkhalf;
	bool diag_mode = drm_cmn_is_diag_mode();;
#endif /* CONFIG_SHARP_DISPLAY */
...
#ifdef CONFIG_SHARP_DISPLAY /* CUST_ID_00024 */ /* CUST_ID_00040 */
		loop++;
		mipiclkhalf = utils->read_bool(child_np, "sharp,mdss-dsi-panel-mipiclkhalf");
		if (diag_mode && mipiclkhalf) {
			num_mipiclkhalf_modes++;
		} else
		if (diag_mode && loop > 1) {
...
#ifdef CONFIG_SHARP_DISPLAY /* CUST_ID_00024 */
	panel->num_display_modes += num_mipiclkhalf_modes;
#endif /* CONFIG_SHARP_DISPLAY */
...
#ifndef CONFIG_SHARP_DISPLAY /* CUST_ID_00008 */
	/* If LP11_INIT is set, panel will be powered up during prepare() */
	if (panel->lp11_init)
		goto error;
#endif /* CONFIG_SHARP_DISPLAY */
...
#ifdef CONFIG_SHARP_DISPLAY /* CUST_ID_00016 */ /* CUST_ID_00036 */
	if (!rc) {
		panel->mfr = MFR_INIT_VALUE_PANEL_ON;
		panel->bias_register = drm_bias_get_otp_voltage();
	}
#endif /* CONFIG_SHARP_DISPLAY */

#ifdef CONFIG_SHARP_DISPLAY /* CUST_ID_00037 */
	if (drm_cmn_get_panel_type() == 0) {
		rc = dsi_panel_tx_cmd_set(panel, DSI_CMD_SET_ON_EVT);
		if (rc)
			DSI_ERR("[%s] failed to send DSI_CMD_SET_ON_EVT cmds, rc=%d\n",
			       panel->name, rc);
	}
	if (drm_cmn_get_panel_type() < 2) {
		rc = dsi_panel_tx_cmd_set(panel, DSI_CMD_SET_ON_DVT);
		if (rc)
			DSI_ERR("[%s] failed to send DSI_CMD_SET_ON_DVT cmds, rc=%d\n",
			       panel->name, rc);
	}
#endif /*  CONFIG_SHARP_DISPLAY */

#ifdef CONFIG_SHARP_DISPLAY /* CUST_ID_00053 */
	rc = drm_gmm_active_gamma_table(panel);
	if (rc) {
		DSI_ERR("[%s] failed to active gamma table, rc=%d\n",
		       panel->name, rc);
	}
#endif /* CONFIG_SHARP_DISPLAY */
...
#ifdef CONFIG_SHARP_DISPLAY /* CUST_ID_00036 */
	drm_bias_enable(false);
#endif /* CONFIG_SHARP_DISPLAY */
#ifdef CONFIG_SHARP_DISPLAY /* CUST_ID_00042 */
	drm_voltage_control_enable(false);
#endif /* CONFIG_SHARP_DISPLAY */
#ifdef CONFIG_SHARP_DISPLAY /* CUST_ID_00028 */
	bd = backlight_device_get_by_type(BACKLIGHT_RAW);
	if (bd) {
		...
		DSI_ERR("backlight_device is NULL\n");
	}
#endif /* CONFIG_SHARP_DISPLAY */
#ifdef CONFIG_SHARP_DISPLAY /* CUST_ID_00049 */
	drm_peak_brightness_ctrl_enable(false);
#endif /* CONFIG_SHARP_DISPLAY */
		...
		struct dsi_panel_linear_params *linear_params)
{
	...
	if (!linear_params) {
		DSI_DEBUG("%s:linear_params is null\n", __func__);
		goto param_err;
	}

	cnt = linear_params->cnt;
	DSI_DEBUG("%s:param cnt:%d\n", __func__, cnt);
	for (i = 0;i < cnt;i++) {
		DSI_DEBUG("%s:param %d x:%d y:%d\n", __func__,
...
#ifdef CONFIG_SHARP_DISPLAY /* CUST_ID_00052 */
static int dsi_panel_parse_brightness_sub(struct dsi_panel_linear_params *linear_params,
	struct device_node *of_node)
{
	int rc, i;
	int size = 0, cnt;
	...
	if (of_get_property(of_node,
		...
		DSI_ERR("%s:Property linear-params not available\n", __func__);
		return 0;
	}

	if (!size) {
		DSI_ERR("%s:Property linear-params size = 0\n", __func__);
		return 0;
	}

	linear_params->param = kzalloc(size, GFP_KERNEL);
	if (!linear_params->param) {
		DSI_ERR("%s:Failed to alloc mem for linear-params\n",__func__);
		return 0;
	}

	rc = of_property_read_u32_array(of_node,
		"sharp,dsi-linear-params",
		(u32 *)linear_params->param, cnt);
	if (rc) {
		DSI_ERR("%s:Error in reading property: linear-params\n",
			__func__);
		return 0;
	}
	linear_params->cnt = cnt / 2;

	DSI_DEBUG("%s:linear_params cnt:%d\n", __func__, linear_params->cnt);
	for (i = 0;i < linear_params->cnt;i++) {
		DSI_DEBUG("%s:linear_param %d x:%d y:%d\n", __func__, i,
			linear_params->param[i].x,
			linear_params->param[i].y);
	}
...
static int dsi_panel_parse_brightness(struct dsi_panel *panel,
	struct device_node *of_node)
{
	struct device_node *linearparams_np, *child_np;
	struct dsi_parser_utils *utils;
	int rc = 0, num_linearparams;
	int cnt = 0;

	utils = &panel->utils;

	linearparams_np = utils->get_child_by_name(utils->data,
		"sharp,dsi-linear-params-entries");
	if (!linearparams_np) {
		DSI_ERR("no display linear-params nodes defined\n");
		rc = -EINVAL;
	}

	num_linearparams = utils->get_child_count(linearparams_np);

	if (!num_linearparams || num_linearparams > MSM_DISPLAY_DRIVE_MODE_HIGH_BRIGHTNESS) {
		DSI_ERR("invalid count of linear-params nodes: %d\n", num_linearparams);
		return -EINVAL;
	}

	dsi_for_each_child_node(linearparams_np, child_np) {
		if (cnt < MSM_DISPLAY_DRIVE_MODE_HIGH_BRIGHTNESS) {
			dsi_panel_parse_brightness_sub(&panel->linear_params_table[cnt], child_np);
			cnt++;
		}
	}

	memcpy(&panel->linear_params, &panel->linear_params_table[0],
		sizeof(struct dsi_panel_linear_params));
...
int dsi_panel_set_mfr(struct dsi_panel *panel, int mfr)
{
	...
	if (panel->mfr == mfr) {
		DSI_DEBUG("ignore duplicated mfr(%d) setting\n", mfr);
		goto error;
	}

	switch (mfr) {
	case 60:
		rc = dsi_panel_tx_cmd_set(panel, DSI_CMD_SET_MFR_60HZ);
		if (panel->mfr == 30) {
			drm_redraw_event_notify();
		}
		break;
	case 30:
		rc = dsi_panel_tx_cmd_set(panel, DSI_CMD_SET_MFR_30HZ);
		break;
	case 1:
		rc = dsi_panel_tx_cmd_set(panel, DSI_CMD_SET_MFR_1HZ);
		break;
	default:
		DSI_ERR("mfr num(%d) not supported\n", mfr);
		rc = -ENOTSUPP;
		goto error;
	}

	if (rc) {
		DSI_ERR("[%s] failed to send DSI_CMD_SET_MFR(%d) cmds, rc=%d\n",
		       panel->name, mfr, rc);
		goto error;
	}
...
#ifdef CONFIG_SHARP_DISPLAY /* CUST_ID_00033 */ /* CUST_ID_00036 */ /* CUST_ID_00048 */ /* CUST_ID_00049 */ /* CUST_ID_00052 */
static int dsi_panel_drivemode_vgsp_set(struct dsi_panel *panel,
		enum msm_drive_mode mode)
{
	struct dsi_display *pdisp = NULL;
	unsigned char data = 0;
	unsigned char addr_value_page[] = {0xF0, 0x55, 0xAA, 0x52, 0x08, 0x01};
	unsigned char addr_value_vgsp[] = {0xB6, 0x00, 0x00};
	struct dsi_cmd_desc vgsp_dp_cmd[] = {
		{{0, MIPI_DSI_GENERIC_LONG_WRITE, 0, 0, 0,
			sizeof(addr_value_page), addr_value_page,
			0, NULL}, 0, 0},
		{{0, MIPI_DSI_GENERIC_LONG_WRITE, 0, 0, 0,
			sizeof(addr_value_vgsp), addr_value_vgsp,
			0, NULL}, 0, 0},
	};
	...
	pdisp = container_of(panel->host, struct dsi_display, host);
	if (!pdisp) {
			DSI_ERR("%s: dsi_display is NULL.\n", __func__);
		return -EINVAL;
	}

	data = drm_cmn_get_otp_vgsp();
	DSI_DEBUG("%s: vgsp update(0x%02x)\n", __func__, data);
	addr_value_vgsp[2] = data;

	switch (mode) {
	case MSM_DISPLAY_DRIVE_MODE_NORMAL:
	case MSM_DISPLAY_DRIVE_MODE_GRADATION_12BIT:
		vgsp_dp_cmd[1].last_command = true;
			break;
		default:
			break;
	}

	return drm_cmn_dsi_cmds_transfer(pdisp, vgsp_dp_cmd,
		ARRAY_SIZE(vgsp_dp_cmd));
}

int dsi_panel_set_drive_mode(struct dsi_panel *panel,
		enum msm_drive_mode mode)
{
	int rc = 0;
	int ret = 0;
	...
	if (cancel_delayed_work_sync(&drive_ctx.delayedwk) == true) {
		DSI_DEBUG("%s: cancel_delayed_work done.\n", __func__);
	}
	...
	if (atomic_read(&panel->esd_recovery_pending)) {
		DSI_DEBUG("ESD recovery pending\n");
		panel->drive_mode = MSM_DISPLAY_DRIVE_MODE_DISP_OFF;
		if (dev) {
			drm_notifier_drive_mode(dev);
		}
		rc = -EINVAL;
		goto end;
	}

	DSI_DEBUG("[%s] drive mode(%d)\n", panel->name, mode);

	if (panel->drive_mode == mode) {
		DSI_DEBUG("[%s] ignore duplicated drive mode(%d) setting\n", panel->name, mode);
		goto end;
	}

	if (mode == MSM_DISPLAY_DRIVE_MODE_DISP_OFF) {
		dsi_panel_set_drive_mode_sub(panel, mode);
		goto end;
	}

	if (panel->drive_mode == MSM_DISPLAY_DRIVE_MODE_SUPER_BRIGHT) {
		change_wait = drivemode_delayed_ms_exit_mode3;
	} else if (mode == MSM_DISPLAY_DRIVE_MODE_SUPER_BRIGHT) {
		change_wait = drivemode_delayed_ms_enter_mode3;
	} else {
		change_wait = drivemode_delayed_ms_other;
	}

	panel->request_drive_mode = mode;
	ret = queue_delayed_work(drive_ctx.delayedwkq,
			&drive_ctx.delayedwk,
			msecs_to_jiffies(change_wait));
	if (ret == 0) {
		DSI_DEBUG("%s: failed to queue_work(). ret=%d\n", __func__, ret);
	}
		...
		enum msm_drive_mode mode)
{
	...
	before_mode = panel->drive_mode;
	panel->drive_mode = mode;
	...
	case MSM_DISPLAY_DRIVE_MODE_DISP_OFF:
		...
		break;
	case MSM_DISPLAY_DRIVE_MODE_NORMAL:
		if (before_mode != MSM_DISPLAY_DRIVE_MODE_DISP_OFF) {
			rc = dsi_panel_tx_cmd_set(panel, DSI_CMD_SET_DRIVE_MODE_NORMAL);
			rc |= dsi_panel_drivemode_vgsp_set(panel, mode);
		}
		break;
	case MSM_DISPLAY_DRIVE_MODE_GAME:
		rc = dsi_panel_tx_cmd_set(panel, DSI_CMD_SET_DRIVE_MODE_NORMAL);
		rc |= dsi_panel_drivemode_vgsp_set(panel, mode);
		rc |= dsi_panel_tx_cmd_set(panel, DSI_CMD_SET_DRIVE_MODE_GAME);
		break;
	case MSM_DISPLAY_DRIVE_MODE_SUPER_BRIGHT:
		rc = dsi_panel_tx_cmd_set(panel, DSI_CMD_SET_DRIVE_MODE_NORMAL);
		rc |= dsi_panel_drivemode_vgsp_set(panel, mode);
		rc |= dsi_panel_tx_cmd_set(panel, DSI_CMD_SET_DRIVE_MODE_SUPER_BRIGHT);
		break;
	case MSM_DISPLAY_DRIVE_MODE_GRADATION_12BIT:
		rc = dsi_panel_tx_cmd_set(panel, DSI_CMD_SET_DRIVE_MODE_NORMAL);
		rc |= dsi_panel_drivemode_vgsp_set(panel, mode);
	...
	case MSM_DISPLAY_DRIVE_MODE_HIGH_BRIGHTNESS:
		rc = dsi_panel_tx_cmd_set(panel, DSI_CMD_SET_DRIVE_MODE_NORMAL);
		rc |= dsi_panel_drivemode_vgsp_set(panel, mode);
		rc |= dsi_panel_tx_cmd_set(panel, DSI_CMD_SET_DRIVE_MODE_HIGH_BRIGHTNESS);
		break;
	default:
		DSI_ERR("drive mode num(%d) not supported\n", mode);
		rc = -ENOTSUPP;
		goto error;
	}

	if (rc) {
		DSI_ERR("[%s] failed to send DSI_CMD_SET_DRIVE_MODE(%d) cmds, rc=%d\n",
		       panel->name, mode, rc);
		panel->drive_mode = before_mode;
		if (dev) {
			drm_notifier_drive_mode(dev);
		}
		goto error;
	...
	if (mode != MSM_DISPLAY_DRIVE_MODE_DISP_OFF) {
		memcpy(&panel->linear_params, &panel->linear_params_table[mode-1],
			sizeof(struct dsi_panel_linear_params));
		dsi_panel_update_backlight_drive_mode(panel);
	}

	switch (mode) {
	case MSM_DISPLAY_DRIVE_MODE_DISP_OFF:
		drm_bias_clear_duty_gmm(panel);
		DSI_DEBUG("%s: drive_mode off.\n", __func__);
		break;
	case MSM_DISPLAY_DRIVE_MODE_NORMAL:
	case MSM_DISPLAY_DRIVE_MODE_GAME:
	case MSM_DISPLAY_DRIVE_MODE_GRADATION_12BIT:
		rc = drm_bias_set_duty_gmm(panel);
		if (rc) {
			DSI_ERR("[%s] failed to calc duty and gmm, rc=%d\n", __func__, rc);
			goto error;
		}
		drm_bias_best_bias_setting(panel);
		break;
	default:
		...
		case MSM_DISPLAY_DRIVE_MODE_GAME:
		case MSM_DISPLAY_DRIVE_MODE_GRADATION_12BIT:
			drm_bias_default_bias_setting(panel);
			break;
		default:
			break;
		}
...
static void dsi_panel_drive_mode_work(struct work_struct *work)
{
	...
	enum msm_drive_mode mode = 0;
	int rc = 0;

	panel = drive_ctx.panel;
	...
	rc = dsi_panel_set_drive_mode_sub(panel, mode);
	if (rc) {
		DSI_ERR("[%s] failed to dsi_panel_set_drive_mode_sub(%d) cmds, rc=%d\n",
				panel->name, mode, rc);
		goto error;
	}
error:
	mutex_unlock(&panel->panel_lock);
	if (mode == MSM_DISPLAY_DRIVE_MODE_SUPER_BRIGHT) {
		drm_peak_brightness_ctrl_enable(true);
	} else {
		drm_peak_brightness_ctrl_enable(false);
	}
	if (!rc) {
...
void dsi_panel_drive_mode_init(struct msm_drm_private *priv)
{
	...
	drive_ctx.delayedwkq = create_singlethread_workqueue("drive_mode_work");
	if (IS_ERR_OR_NULL(drive_ctx.delayedwkq)) {
		DSI_ERR("%s: Error creating delayedwkq\n", __func__);
		return;
	}
	...
	if (kms != NULL) {
		...
		if ((sde_kms->dsi_display_count > 0) &&
			...
			if (display == NULL) {
				DSI_ERR("[%s]failed to display is null\n", __func__);
			} else if (display->panel) {
				drive_ctx.panel = display->panel;
			}
		} else {
			DSI_ERR("[%s]failed to dsi_displays count=%d addr=%p\n",
				__func__, sde_kms->dsi_display_count,
				sde_kms->dsi_displays);
		}
	} else {
		DSI_ERR("[%s]failed to kms is null\n", __func__);
	}

	INIT_DELAYED_WORK(&drive_ctx.delayedwk, dsi_panel_drive_mode_work);
	return;
}
#endif /* CONFIG_SHARP_DISPLAY */

#ifdef CONFIG_SHARP_DISPLAY /* CUST_ID_00031 */ /* CUST_ID_00036 */ /* CUST_ID_00048 */ /* CUST_ID_00052 */
static struct dsi_panel *to_dsi_panel(struct device *dev)
{
...
static int dsi_panel_get_dev_brightness(struct dsi_panel *panel, u32 *brightness)
{
	struct dsi_backlight_config *bl = NULL;

	if (!panel) {
		DSI_ERR("%s: Invalid input data\n", __func__);
		return -EINVAL;
	}
	...
	*brightness = bl->dev_brightness;
	return 0;
}

static int dsi_panel_get_boost(struct dsi_panel *panel,
...
static int dsi_panel_request_boost(struct dsi_panel *panel, u32 onoff)
{
	struct dsi_backlight_config *bl = NULL;
	...
	if ((onoff) && (bl->last_level == 0)) {
		DSI_DEBUG("%s: skip curr_boost on\n",__func__);
		return 0;
	}
	...
	case 0:
		...
		break;
	case 1:
		bl->boost_mode = DSI_INCAMERA_BOOST_ON;
		break;
	default:
		DSI_ERR("boost_mode chaneged %d Error\n", onoff);
		mutex_unlock(&panel->curr_boost_lock);
		return 0;
	}
...
static int dsi_panel_bl_boost(struct dsi_panel *panel, u32 onoff)
{
	...
	if (bl->boost_mode >= DSI_BOOST_MODE_MAX) {
		DSI_ERR("%s: Invalid boost_mode data\n",__func__);
		return -EINVAL;
	}
	...
	if (onoff) {
		// curr_boost on
		bl_lvl = boost_brightness[bl->boost_mode];
	} else {
		// curr_boost off
		bl_lvl = bl->last_level;
	}
...
static void dsi_panel_curr_boost_panel_mutex_lock(enum dsi_backlight_type type,
										struct dsi_panel *panel)
{
	if (!panel) {
		return;
...
static void dsi_panel_curr_boost_panel_mutex_unlock(enum dsi_backlight_type type,
										struct dsi_panel *panel)
{
	if (!panel) {
		return;
...
static void dsi_panel_curr_boost_work(struct work_struct *work)
{
	...
	dsi_panel_curr_boost_panel_mutex_lock(bl->type, panel);
	mutex_lock(&panel->curr_boost_lock);

	if (bl->boost_mode >= DSI_BOOST_MODE_MAX) {
		DSI_ERR("%s: Invalid boost_mode data\n",__func__);
		mutex_unlock(&panel->curr_boost_lock);
		dsi_panel_curr_boost_panel_mutex_unlock(bl->type, panel);
		return;
	}

	if (bl->curr_boost_req == true) {
		bl->curr_boost_req = false;
		DSI_DEBUG("resuest mode:%d\n", bl->boost_mode);
		if (bl->last_level) {
			//set curr_boost on
			rc = dsi_panel_bl_boost(panel, true);
			if (!rc) {
				dsi_panel_update_curr_boosted(panel, true);
				queue_delayed_work(panel->ordered_workqueue,
						&panel->curr_boost_work,
						msecs_to_jiffies(cancel_delay_ms[bl->boost_mode]));
			} else {
				DSI_ERR("%s: curr_boost failed err=%d.\n",
							__func__, rc);
			}
		}
	}
...
int dsi_panel_boost_work_init(struct dsi_panel *panel, bool Initialize)
{
	int rc = 0;
	...
	if (Initialize) {
		...
		if (!panel->ordered_workqueue) {
			rc = -ENOMEM;
			DSI_ERR("%s: alloc_ordered_workqueue failed\n", __func__);
			return rc;
		}
...
static ssize_t dsi_panel_curr_boost_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct dsi_panel *panel = to_dsi_panel(dev);
	int boost_req = 0;
	int boosted = 0;

	if (panel) {
		dsi_panel_get_boost(panel, &boost_req, &boosted);
	}
...
static ssize_t dsi_panel_curr_boost_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (panel) {
		rc = dsi_panel_request_boost(panel, data);
	}
...
static DEVICE_ATTR(curr_boost, 0664,
		dsi_panel_curr_boost_show, dsi_panel_curr_boost_store);

static ssize_t dsi_panel_dev_brightness_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct dsi_panel *panel = to_dsi_panel(dev);
	...
	if (panel) {
		dsi_panel_get_dev_brightness(panel, &brightness);
	}
...
/* sysfs attributes exported by dev_brightness */
static DEVICE_ATTR(dev_brightness, 0664,
		dsi_panel_dev_brightness_show, NULL);

static ssize_t dsi_panel_duty_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct dsi_panel *panel = to_dsi_panel(dev);
	int duty = 0;
...
/* sysfs attributes exported by duty */
static DEVICE_ATTR(duty, 0664,
		dsi_panel_duty_show, NULL);

static struct attribute *bl_device_attrs[] = {
	&dev_attr_curr_boost.attr,
	&dev_attr_dev_brightness.attr,
	&dev_attr_duty.attr,
	NULL,
};

static struct attribute_group bl_device_group = {
	.name = "backlight",
...
static int dsi_panel_create_sysfs(struct dsi_panel *panel)
{
	...
	if (!panel || !panel->parent) {
		DSI_ERR("%s: Invalid dsi_panel\n", __func__);
		return -EINVAL;
	}

	dev = panel->parent;
	if (dev) {
		DSI_DEBUG("%s: device_name = [%s]\n", __func__, dev->kobj.name);
		rc = sysfs_create_group(&dev->kobj,
					&bl_device_group);
		if (rc) {
			DSI_ERR("%s: sysfs group creation failed, rc=%d\n", __func__, rc);
		}
	}
...
static void dsi_panel_remove_sysfs(struct dsi_panel *panel)
{
	...
	dev = panel->parent;
	if (dev) {
		sysfs_remove_group(&dev->kobj, &bl_device_group);
	}
}
#endif /* CONFIG_SHARP_DISPLAY */
...
static int dsi_panel_parse_therm(struct dsi_panel *panel)
{
	...
	rc = of_property_match_string(dev->of_node,
			"io-channel-names", "pm8350_lcd_therm");
	if (rc < 0) {
		DSI_ERR("%s: of_property_match_string rc=%d\n", __func__, rc);
		return 0;
	}

	DSI_DEBUG("%s: device_name = [%s]\n", __func__, dev->kobj.name);
	panel->display_therm = iio_channel_get(dev, "pm8350_lcd_therm");
	if (IS_ERR(panel->display_therm)) {
		rc = PTR_ERR(panel->display_therm);
		if (rc != -EPROBE_DEFER)
			DSI_ERR("%s: error iio_channel_get\n", __func__);
		panel->display_therm = NULL;
	}
