...
static const struct drm_prop_enum_list e_drive_mode[] = {
	{MSM_DISPLAY_DRIVE_MODE_DISP_OFF, "disp_off"},
	{MSM_DISPLAY_DRIVE_MODE_NORMAL, "normal"},
	{MSM_DISPLAY_DRIVE_MODE_GAME, "game"},
	{MSM_DISPLAY_DRIVE_MODE_SUPER_BRIGHT, "super_bright"},
	{MSM_DISPLAY_DRIVE_MODE_GRADATION_12BIT, "gradation_12bit"},
	{MSM_DISPLAY_DRIVE_MODE_HIGH_BRIGHTNESS, "high_brightness"},
};
#endif /* CONFIG_SHARP_DISPLAY */
...
static int _sde_connector_update_power_locked(struct sde_connector *c_conn)
{
	...
	mutex_unlock(&c_conn->lock);
#ifdef CONFIG_SHARP_DISPLAY /* CUST_ID_00042 */ /* CUST_ID_00036 */ /* CUST_ID_00049 */
	if (mode != SDE_MODE_DPMS_ON) {
		if (c_conn->ops.voltage_control_enable) {
			c_conn->ops.voltage_control_enable(false);
		}
		if (c_conn->ops.bias_enable) {
			c_conn->ops.bias_enable(false);
		}
		if (c_conn->ops.peak_brightness_ctrl_enable) {
			c_conn->ops.peak_brightness_ctrl_enable(false);
		}
		sde_connector_schedule_status_work(connector, false);
	} else {
		sde_connector_schedule_status_work(connector, true);
		if (c_conn->ops.peak_brightness_ctrl_enable) {
			c_conn->ops.peak_brightness_ctrl_enable(true);
		}
		if (c_conn->ops.bias_enable) {
			c_conn->ops.bias_enable(true);
		}
		if (c_conn->ops.voltage_control_enable) {
			c_conn->ops.voltage_control_enable(true);
		}
	}
#else /* CUST_ID_00042 */ /* CUST_ID_00036 */ /* CUST_ID_00049 */
	if (mode != SDE_MODE_DPMS_ON)
		sde_connector_schedule_status_work(connector, false);
	else
		sde_connector_schedule_status_work(connector, true);
				...
				struct drm_connector *connector)
{
	...
	while ((idx = msm_property_pop_dirty(&c_conn->property_info,
		...
		case CONNECTOR_PROP_HDR_METADATA:
			...
			break;
#ifdef CONFIG_SHARP_DISPLAY /* CUST_ID_00033 */
		case CONNECTOR_PROP_DRIVE_MODE:
			if (c_conn->ops.set_drive_mode) {
				enum msm_drive_mode mode;

				mode = sde_connector_get_property(
						connector->state, CONNECTOR_PROP_DRIVE_MODE);
				c_conn->ops.set_drive_mode(c_conn->display, mode);
			}
			break;
#endif /* CONFIG_SHARP_DISPLAY */
		default:
...
#ifdef CONFIG_SHARP_DISPLAY /* CUST_ID_00033 */
	if (connector_type == DRM_MODE_CONNECTOR_DSI) {
		msm_property_install_enum(&c_conn->property_info, "drive_mode",
				0, 0, e_drive_mode,
				ARRAY_SIZE(e_drive_mode),
				MSM_DISPLAY_DRIVE_MODE_NORMAL,
				CONNECTOR_PROP_DRIVE_MODE);
	}
#endif /* CONFIG_SHARP_DISPLAY */
