		...
		struct sde_kms *sde_kms)
{
	static const struct sde_connector_ops dsi_ops = {
		...
		.set_drive_mode = dsi_display_set_drive_mode,
#endif /* CONFIG_SHARP_DISPLAY */
#ifdef CONFIG_SHARP_DISPLAY /* CUST_ID_00042 */
		.voltage_control_enable = drm_voltage_control_enable,
#endif /* CONFIG_SHARP_DISPLAY */
#ifdef CONFIG_SHARP_DISPLAY /* CUST_ID_00036 */
		.bias_enable = drm_bias_enable,
#endif /* CONFIG_SHARP_DISPLAY */
#ifdef CONFIG_SHARP_DISPLAY /* CUST_ID_00049 */
		.peak_brightness_ctrl_enable = drm_peak_brightness_ctrl_enable,
#endif /* CONFIG_SHARP_DISPLAY */
	};
...
static int sde_kms_set_mfr(const struct msm_kms *kms,
		int mfr)
{
	struct sde_kms *sde_kms = NULL;
	void *display = NULL;
	...
	for (i = 0; i < sde_kms->dsi_display_count; ++i) {
		...
		rc = dsi_display_set_mfr(dsi_display, mfr);
		if (rc) {
			SDE_ERROR("Failed: set mfr rc=%d\n", rc);
			break;
		}
	}
