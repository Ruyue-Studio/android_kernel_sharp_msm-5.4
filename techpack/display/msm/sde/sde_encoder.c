	...
	u32 sw_event)
{
	...
	if(sde_encoder_is_dsi_display(drm_enc)) {
		drm_oneshot_cancel_delayed_work();
	}
#endif /* CONFIG_SHARP_DISPLAY */
...
#ifdef CONFIG_SHARP_DISPLAY /* CUST_ID_00048 */
	if(sde_encoder_is_dsi_display(drm_enc)) {
		drm_oneshot_request_delayed_work();
	}
#endif /* CONFIG_SHARP_DISPLAY */
...
#ifdef CONFIG_SHARP_DISPLAY /* CUST_ID_00040 */
static int vsync_reduce = 1;
#ifdef CONFIG_DEBUG_FS
module_param(vsync_reduce, int, 0600);
...
#ifdef CONFIG_SHARP_DISPLAY /* CUST_ID_00040 */
	if(!vsync_reduce) goto exit;

	if(sde_encoder_is_dsi_display(drm_enc)) {
		struct msm_drm_private *priv = NULL;
		int fps_low_base = DRM_BASE_FPS_DEFAULT;
		int refresh_rate = DRM_BASE_MAX_FPS;
		struct sde_connector *c_conn = NULL;
		struct dsi_display *display = NULL;

		if (drm_enc->dev)
			priv = drm_enc->dev->dev_private;
		if (phy_enc->connector)
			c_conn = to_sde_connector(phy_enc->connector);
		if (c_conn)
		...
		refresh_rate = (refresh_rate > 0) ?
						refresh_rate : DRM_BASE_MAX_FPS;

		if (display->panel->cur_mode->timing.h_skew) {
			fps_low_base = display->panel->cur_mode->timing.h_skew;
		} else {
			fps_low_base = refresh_rate;
		}
		fps_low_base = (fps_low_base > 0) ?
						fps_low_base : DRM_BASE_MAX_FPS;

		if (priv) {
			switch(priv->restrict_fps) {
			case DRM_BASE_FPS_60:
				fps_low_base = DRM_BASE_FPS_60;
				break;
			case DRM_BASE_FPS_30:
				fps_low_base = DRM_BASE_FPS_30;
				break;
			default:
				break;
			}
		...
		phy_enc->fpslow_count++;

		pr_debug("%s: refresh_rate = %d, fps_base = %d, fps_count = %d\n",
				__func__, refresh_rate, fps_low_base, phy_enc->fpslow_count);

		if ((refresh_rate / fps_low_base) > phy_enc->fpslow_count) {
