...
#if defined(CONFIG_ARCH_JUDAU)
#define USES_PANEL_OLAF
#elif defined(CONFIG_ARCH_DOZLE)
...
#elif defined(USES_PANEL_OLAF)
#define DRM_DIAG_DEFAULT_PAGE 0x00
#define DRM_DIAG_PAGE_ADDR 0xF0
#elif defined(USES_PANEL_STEIN)
#define DRM_DIAG_DEFAULT_PAGE 0x00
...
#if defined(USES_PANEL_OLAF)
	{
		unsigned char addr_value_page[] = {0x00, 0x55, 0xAA, 0x52, 0x08, 0x00};
				...
				struct drm_mipichk_param *drm_mipi_check_param)
{
	...
	if (!dsi_panel_initialized(pdisp->panel)) {
...
#if !defined(USES_PANEL_RM69350) && !defined(USES_PANEL_ELSA) && !defined(USES_PANEL_STEIN) && !defined(USES_PANEL_OLAF)
	if (!dsi1_ctrl) {
...
#if !defined(USES_PANEL_RM69350) && !defined(USES_PANEL_STEIN) && !defined(USES_PANEL_OLAF)
...
#else /* DRM_DIAG_PANEL_VIDEO_MODE */
static int drm_diag_atomic_duplicate_state(struct drm_device *dev, int frame_cnt)
{
	int i = 0;
	int ret = 0;
			...
			struct dsi_display *pdisp)
{
	...
	drm_diag_atomic_duplicate_state(drm_mipi_check_param->dev, frame_cnt);
	// 2flame wait
