...
#endif /* CONFIG_SHARP_DISPLAY */
#ifdef CONFIG_SHARP_DISPLAY /* CUST_ID_00044 */
...
#endif /* CONFIG_SHARP_DISPLAY */
#ifdef CONFIG_SHARP_DISPLAY /* CUST_ID_00042 */
	drm_voltage_control_init(dev, priv);
#endif /* CONFIG_SHARP_DISPLAY */
#ifdef CONFIG_SHARP_DISPLAY /* CUST_ID_00048 */
	drm_oneshot_init(dev, priv);
#endif /* CONFIG_SHARP_DISPLAY */
#ifdef CONFIG_SHARP_DISPLAY /* CUST_ID_00049 */
	drm_peak_brightness_ctrl_init(dev, priv);
#endif /* CONFIG_SHARP_DISPLAY */
#ifdef CONFIG_SHARP_DISPLAY /* CUST_ID_00033 */
	dsi_panel_drive_mode_init(priv);
#endif /* CONFIG_SHARP_DISPLAY */
...
#ifdef CONFIG_SHARP_DISPLAY /* CUST_ID_00016 */
static int msm_ioctl_set_mfr(struct drm_device *dev, void *data,
		struct drm_file *file_priv)
{
	struct msm_drm_private *priv;
	struct msm_kms *kms;
	...
	if (kms && kms->funcs && kms->funcs->set_mfr) {
		kms->funcs->set_mfr(kms, mfr);
	}

	pr_debug("%s: set mfr=%d\n", __func__, mfr);
...
#endif /* CONFIG_SHARP_DISPLAY */
#ifdef CONFIG_SHARP_DISPLAY /* CUST_ID_00016 */
	DRM_IOCTL_DEF_DRV(SET_MFR, msm_ioctl_set_mfr,
			DRM_UNLOCKED),
#endif /* CONFIG_SHARP_DISPLAY */
};
...
#ifdef CONFIG_SHARP_DISPLAY /* CUST_ID_00003 */ /* CUST_ID_00041 */
int msm_drm_check_upper_unit(struct msm_drm_private *priv)
{
	unsigned char val = 0;
	...
	val = drm_cmn_get_upperunit();
	if (!val) {
		DRM_WARN("Upper Unit not connected. upper_unit=%d\n", val);
		priv->upper_unit_is_connected = DRM_UPPER_UNIT_IS_NOT_CONNECTED;
		return -EIO;
	}
	DRM_DEBUG("check_upper_unit connected=%d\n", priv->upper_unit_is_connected);
	return 0;
}
#endif /* CONFIG_SHARP_DISPLAY */
...
#ifdef CONFIG_SHARP_DISPLAY /* CUST_ID_00014 */
static struct dsi_display *msm_drv_device_to_dsi_panel(void)
{
	int count = 0;
		...
		struct drm_file *file)
{
	...
	display = msm_drv_device_to_dsi_panel();
	if (!display)
		return -EINVAL;
	...
	ret = drm_diag_mipi_check(display, &drm_mipi_check_param);
