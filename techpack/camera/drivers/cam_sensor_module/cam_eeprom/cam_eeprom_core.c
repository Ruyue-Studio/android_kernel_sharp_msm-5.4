...
/* SHLOCAL_CAMERA_DRIVERS-> */
			i2c_reg_settings.delay = emap[j].page.sw_delay_msec;
/* SHLOCAL_CAMERA_DRIVERS<- */
...
#else
			i2c_reg_array.delay = emap[j].page.hw_delay_usec;
#endif
/* SHLOCAL_CAMERA_DRIVERS<- */
...
/* SHLOCAL_CAMERA_DRIVERS-> */
			i2c_reg_settings.delay = emap[j].pageen.sw_delay_msec;
/* SHLOCAL_CAMERA_DRIVERS<- */
			i2c_reg_settings.size = 1;
...
#else
			i2c_reg_array.delay = emap[j].pageen.hw_delay_usec;
#endif
/* SHLOCAL_CAMERA_DRIVERS<- */
			i2c_reg_settings.reg_setting = &i2c_reg_array;
			rc = camera_io_dev_write(&e_ctrl->io_master_info,
...
/* SHLOCAL_CAMERA_DRIVERS-> */
		...
		if (generic_op_code == CAMERA_SENSOR_WAIT_OP_HW_UCND) {
			map[*num_map - 1].mem.hw_delay_usec = i2c_uncond_wait->delay;
			map[*num_map - 1].page.hw_delay_usec = i2c_uncond_wait->delay;
			map[*num_map - 1].pageen.hw_delay_usec = i2c_uncond_wait->delay;
		} else if (generic_op_code == CAMERA_SENSOR_WAIT_OP_SW_UCND) {
			map[*num_map - 1].mem.sw_delay_msec = i2c_uncond_wait->delay;
			map[*num_map - 1].page.sw_delay_msec = i2c_uncond_wait->delay;
			map[*num_map - 1].pageen.sw_delay_msec = i2c_uncond_wait->delay;
		}
#endif
/* SHLOCAL_CAMERA_DRIVERS<- */
...
#else
			map[*num_map].poll.sw_delay_msec = i2c_poll->timeout;
#endif
/* SHLOCAL_CAMERA_DRIVERS<- */
