...
#define CS35L41_RECEIVER_NAME "cs35l41.8-0040"
#define CS35L41_SPEAKER_NAME "cs35l41.8-0041"
static atomic_t cs35l41_mclk_rsc_ref;
static int msm_mi2s_snd_startup(struct snd_pcm_substream *substream);
static void msm_mi2s_snd_shutdown(struct snd_pcm_substream *substream);

SND_SOC_DAILINK_DEFS(cs35l41_codec_rx,
	DAILINK_COMP_ARRAY(COMP_CPU("msm-dai-q6-mi2s.0")),
	DAILINK_COMP_ARRAY(
		COMP_CODEC(CS35L41_RECEIVER_NAME, "cs35l41-pcm"),
		COMP_CODEC(CS35L41_SPEAKER_NAME, "cs35l41-pcm")),
	DAILINK_COMP_ARRAY(COMP_PLATFORM("msm-pcm-routing"))
);

SND_SOC_DAILINK_DEFS(cs35l41_codec_tx,
...
error:
	...
	dev_info(card->dev, "-%s, ret = %d, mclk refcount = %d\n", __func__,
		ret, atomic_read(&cs35l41_mclk_rsc_ref));
...
static int cs35l41_init(struct snd_soc_pcm_runtime *rtd)
{
	...
	struct snd_soc_dapm_context *rcv_dapm = snd_soc_component_get_dapm(rtd->codec_dais[0]->component);
	struct snd_soc_dapm_context *spk_dapm = snd_soc_component_get_dapm(rtd->codec_dais[1]->component);

	dev_info(card->dev, "%s: set cs35l41_mclk_rsc_ref to 0 \n", __func__);
	atomic_set(&cs35l41_mclk_rsc_ref, 0);

	dev_info(card->dev, "%s: found codec[%s]\n", __func__, dev_name(rcv_dapm->component->dev));
	...
	dev_info(card->dev, "%s: found codec[%s]\n", __func__, dev_name(spk_dapm->component->dev));
	snd_soc_dapm_ignore_suspend(spk_dapm, "SPK AMP Playback");
	snd_soc_dapm_ignore_suspend(spk_dapm, "SPK AMP Capture");
	snd_soc_dapm_ignore_suspend(spk_dapm, "SPK ASPRX1");
	snd_soc_dapm_ignore_suspend(spk_dapm, "SPK ASPRX2");
	snd_soc_dapm_ignore_suspend(spk_dapm, "SPK ASPTX1");
	snd_soc_dapm_ignore_suspend(spk_dapm, "SPK ASPTX2");
...
#else
//SH_AUDIO_DRIVER/ <- /* A-001 */
		SND_SOC_DAILINK_REG(cs35l41_codec_rx),
//SH_AUDIO_DRIVER/ <- /* A-001 */
		.init = &cs35l41_init,
#endif
/* SH_AUDIO_DRIVER <- A-001 */
...
#else
//SH_AUDIO_DRIVER/ <- /* A-001 */
		SND_SOC_DAILINK_REG(cs35l41_codec_tx),
//SH_AUDIO_DRIVER/ <- /* A-001 */
#endif
/* SH_AUDIO_DRIVER <- A-001 */
	},
