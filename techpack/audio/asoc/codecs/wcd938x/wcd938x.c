...
static int wcd938x_headset_status_get(struct snd_kcontrol *kcontrol,
				 struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *component = snd_soc_kcontrol_component(kcontrol);
	struct wcd938x_priv *wcd938x = snd_soc_component_get_drvdata(component);
	struct wcd_mbhc mbhc = wcd938x->mbhc->wcd_mbhc;
	dev_dbg(component->dev, "mbhc.headset_jack.status is %d",mbhc.headset_jack.status);
	ucontrol->value.integer.value[0] = mbhc.headset_jack.status;
	return 0;
}

static int wcd938x_headset_status_put(struct snd_kcontrol *kcontrol,
				 struct snd_ctl_elem_value *ucontrol)
{
	return 0;
}

static int wcd938x_button_status_get(struct snd_kcontrol *kcontrol,
				 struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *component = snd_soc_kcontrol_component(kcontrol);
	struct wcd938x_priv *wcd938x = snd_soc_component_get_drvdata(component);
	struct wcd_mbhc mbhc = wcd938x->mbhc->wcd_mbhc;
	dev_dbg(component->dev, "mbhc.button_jack.status is %d",mbhc.button_jack.status);
	ucontrol->value.integer.value[0] = mbhc.button_jack.status;
	return 0;
}

static int wcd938x_button_status_put(struct snd_kcontrol *kcontrol,
				 struct snd_ctl_elem_value *ucontrol)
{
	return 0;
}
...
/* SH_AUDIO_DRIVER -> */ /* A-006 */
static const char * const button_status_text[] = {
	"OFF", "ON",
};

static const struct soc_enum button_status_enum =
	SOC_ENUM_SINGLE_EXT(ARRAY_SIZE(button_status_text),
			    button_status_text);

static const char * const headset_status_text[] = {
	"UNINSERTED", "INSERTED",
};

static const struct soc_enum headset_status_enum =
	SOC_ENUM_SINGLE_EXT(ARRAY_SIZE(headset_status_text),
			    headset_status_text);
/* SH_AUDIO_DRIVER <- */ /* A-006 */
...
static const struct snd_kcontrol_new wcd938x_snd_controls[] = {
	...
	SOC_ENUM_EXT("DMIC7 ChMap", tx_master_ch_enum,
			wcd938x_tx_master_ch_get, wcd938x_tx_master_ch_put),
/* SH_AUDIO_DRIVER -> */ /* A-006 */
	SOC_ENUM_EXT("WCD Headset status", headset_status_enum,
			wcd938x_headset_status_get, wcd938x_headset_status_put),
	SOC_ENUM_EXT("WCD button status", button_status_enum,
			wcd938x_button_status_get, wcd938x_button_status_put),
/* SH_AUDIO_DRIVER <- */ /* A-006 */
};
...
static int wcd938x_bind(struct device *dev)
{
	...
	if (ret) {
		dev_err(dev, "%s: Slave bind failed, ret = %d\n",
			__func__, ret);
/* SH_AUDIO_DRIVER -> */ /* R#346516 */
		if (ret == -EPROBE_DEFER && (++num_wcd938x_bind >= NUM_WCD938X_BIND_MAX)) {
			dev_err(dev, "%s:  restart system", __func__);
			num_wcd938x_bind = 0;
			panic("reboot system");
		}
/* SH_AUDIO_DRIVER <- */ /* R#346516 */
