		...
		struct snd_kcontrol *kcontrol, int event)
{
	...
	case SND_SOC_DAPM_POST_PMU:
#ifdef CONFIG_SH_AUDIO_DRIVER /* B-019 */
		dev_dbg(cs35l41->dev, "%s component->name_prefix is %s\n", __func__,component->name_prefix);
		if(!strncmp(component->name_prefix, "SPK", 1)){
			dev_dbg(cs35l41->dev, "%s Stop pedometer function\n", __func__);
			shub_api_audio_pedometer_func(1);
		}
#endif /* CONFIG_SH_AUDIO_DRIVER */ /* B-019 */
	...
	case SND_SOC_DAPM_POST_PMD:
		...
		dev_dbg(cs35l41->dev, "%s component->name_prefix is %s\n", __func__,component->name_prefix);
		if(!strncmp(component->name_prefix, "SPK", 1)){
			dev_dbg(cs35l41->dev, "%s Restart pedometer function\n", __func__);
			shub_api_audio_pedometer_func(0);
		}
#endif /* CONFIG_SH_AUDIO_DRIVER */ /* B-019 */
		if (cs35l41->dsp.running) {
...
/* Add for avoid Android GKI Start */
const char *cs35l41_devm_kstrdup_const(struct device *dev, const char *s, gfp_t gfp)
{
...
/* Add for avoid Android GKI Start */
	cs35l41->dt_name = cs35l41_devm_kstrdup_const(cs35l41->dev, np->name,
					      GFP_KERNEL);
/* Add for avoid Android GKI End */
