...
#ifndef SVERSION_OF_SOFT
#define SVERSION_OF_SOFT ""
#endif /* SVERSION_OF_SOFT */
...
static int shrlog_handle_key_init(struct platform_device *pdev, int force_en)
{
	int ret = 0;
	u32 toval = 0;
	const char* sverstr = SVERSION_OF_SOFT;
	if (enable_pwrkey_panic == 0) {
		if ((sverstr[0] != '\0') && (sverstr[0] != 'S')) {
			  ...
			  unsigned long val, void *data)
{
	...
	if (val == MODULE_STATE_COMING) {
		if (!shrlog_info->module_addr) {
			shrlog_info->module_addr = (shrlog_kernel_t)&(mod->list);
		}
		tried = 1;
	}
	return 0;
 ...
 */
	...
	apps_info->uses_ftrace = 1;
#else /* !CONFIG_FUNCTION_TRACER */
	apps_info->uses_ftrace = 0;
...
static int shrlog_probe(struct platform_device *pdev)
{
	...
	do {
		...
		if (np) {
			...
			if (kaslr_imem_addr) {
				uint32_t read_val;
				read_val = __raw_readl(kaslr_imem_addr + 0);
				iounmap(kaslr_imem_addr);
				if (read_val != 0xdead4ead) {
					return -EPROBE_DEFER;
				}
			}
		}
...
#if defined(CONFIG_SHARP_PANIC_ON_PWRKEY)
	if ((shrlog_ram_fixed->kmagic1 == (shrlog_uint64)SHRLOG_FIXED_MAGIC_NUM_C)
	    && (shrlog_ram_fixed->kmagic2 == (shrlog_uint64)SHRLOG_FIXED_MAGIC_NUM_D))
		k_enabled = 1;
	else
		k_enabled = 0;
#endif /* CONFIG_SHARP_PANIC_ON_PWRKEY */
	shrlog_ram_fixed->kmagic1 = 0;
	shrlog_ram_fixed->kmagic2 = 0;
	...
	shrlog_ram_fixed->linux_phys_offset = 0;
	...
	if (shrlog_handle_keyevent_keycode) {
		ret = shrlog_handle_key_init(pdev, k_enabled);
		if (ret)
	...
	do {
		if (!shrlog_info->module_addr) {
			shrlog_info->module_addr = (shrlog_kernel_t)&(THIS_MODULE->list);
		}
	} while(0);
#else /* !MODULE */
	do {
