...
//DIAG_9999 #include "../../../techpack/display/msm/sharp/msm_drm_context.h"
...
static int smd_ioctl_get_wpflg(unsigned long arg)
{
	int wpflg = 0;
	sharp_smem_common_type *sh_smem = NULL;
	...
	if( sh_smem != 0 )
	{
		wpflg = sh_smem->sh_memory_wpflg;

		if(copy_to_user((int __user *)arg, &wpflg, sizeof(int)) != 0)
		{
			printk("[%s]: copy_to_user FAIL\n",__func__);
			return -EFAULT;
		}
...
#if 0
/* DIAG_1726 add start */
static int smd_ioctl_get_lcd_info(unsigned long arg)
{
	int lcd_info = 0;
	sharp_smem_common_type *sh_smem = NULL;
	...
	if( sh_smem != 0 )
	{
		...
		lcd_info = shdisp_boot_ctx->panel_connected;

		if(copy_to_user((int __user *)arg, &lcd_info, sizeof(int)) != 0)
		{
			printk("[%s]: copy_to_user FAIL\n",__func__);
			return -EFAULT;
		}
...
static long smd_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	case SHDIAG_IOCTL_GET_LCD_UPPER_INFO:
		...
		break;
	case SHDIAG_IOCTL_GET_WPFLG:
		ret = smd_ioctl_get_wpflg(arg);
		break;
#if 0
/* DIAG_1726 add start */
	case SHDIAG_IOCTL_GET_LCD_INFO:
		ret = smd_ioctl_get_lcd_info(arg);
		break;
/* DIAG_1726 add end */
#endif
	default:
