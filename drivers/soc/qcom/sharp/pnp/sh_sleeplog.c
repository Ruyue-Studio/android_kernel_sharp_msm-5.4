...
static ktime_t last_suspend_time = 0;
static ktime_t total_suspend_time = 0;
static ktime_t last_idle_time = 0;

/**
 * [Power collapse time].suspend
 */
static ktime_t last_stime; /* monotonic boottime offset before last suspend */
static ktime_t curr_stime; /* monotonic boottime offset after last suspend */

static int suspend_state_event(struct notifier_block *notifier,
	unsigned long pm_event, void *unused)
{
	...
	case PM_POST_SUSPEND:
		...
		total_suspend_time += ktime_sub(curr_stime, last_stime);
	default:
		break;
	}
	return NOTIFY_DONE;
...
static struct notifier_block suspend_state_notifer_block = {
	.notifier_call = suspend_state_event,
};
...
static char *sh_write_buffer_pm_stats(char *buffer)
{
	...
	int64_t suspend_result = total_suspend_time;
...
static int __init sh_sleeplog_init( void )
{
	...
	if (register_pm_notifier(&suspend_state_notifer_block)) {
		pr_err("%s() register pm notifier error\n", __func__);
		return -1;
	}
...
static void __exit sh_sleeplog_exit(void)
{
	...
	unregister_pm_notifier(&suspend_state_notifer_block);
}
