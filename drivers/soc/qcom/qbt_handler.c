...
/* SHARP_EXTEND [FINGERPRINT-Drv-000] Start */
#include <linux/regulator/consumer.h>
#include <linux/delay.h>
/* SHARP_EXTEND [FINGERPRINT-Drv-000] End */
/* SHARP_EXTEND [FINGERPRINT-Drv-068] Start */
#include <linux/pm_wakeup.h>
/* SHARP_EXTEND [FINGERPRINT-Drv-068] End */
...
static struct pinctrl *qbt_pinctrl;
static struct pinctrl_state *qbt_pin_state;
...
/* SHARP_EXTEND [FINGERPRINT-Drv-000] Start */
int pinctrl_action(struct device *dev, const char *compat, const char *pin_name)
{
	pr_debug("[qbt] %s  compat = %s  pin_name = %s   start  ----\n", __func__, compat, pin_name);
	dev->of_node = of_find_compatible_node(NULL, NULL, compat);

	if (dev->of_node) {
		qbt_pinctrl = devm_pinctrl_get(dev);

		if (IS_ERR(qbt_pinctrl)) {
			pr_err("[qbt] %s: cannot find qbt_pinctrl\n", __func__);
			return pinctrl_action_error;
		}
	} else {
		pr_err("[qbt] %s: can't find compatible node   %s\n", __func__, compat);
		return pinctrl_action_error;
	}
	qbt_pin_state = pinctrl_lookup_state(qbt_pinctrl, pin_name);

	if (IS_ERR(qbt_pin_state)) {
		pr_err("[qbt] %s: can't find  %s\n", __func__, pin_name);
		return pinctrl_action_error;
	}
	pinctrl_select_state(qbt_pinctrl, qbt_pin_state);
	pr_debug("[qbt] %s  compat = %s  pin_name = %s   done ----\n", __func__, compat, pin_name);
	return 0;
}

static int vreg_setup(struct qbt_drvdata *drvdata, const char *name, bool enable)
{
	int i, rc;
	int found = 0;
	...
	for (i = 0; i < ARRAY_SIZE(vreg_conf); i++) {
		...
		pr_info("[qbt] vreg_conf[%d].name = %s\n", i, name);
		if (!strncmp(n, name, strlen(n))) {
			pr_debug("[qbt] Regulator %s is found\n", name);
			found = 1;
			break;
		}
	}
	...
	if (found == 0) {
		pr_err("[qbt] Regulator %s is not found\n", name);
		return -EINVAL;
	}
	...
	if (enable) {
		...
		if (regulator_count_voltages(vreg) > 0) {
			...
			if (rc)
				pr_err("[qbt] Unable to set voltage on %s, %d\n", name, rc);
		}

		rc = regulator_set_load(vreg, vreg_conf[i].ua_load);
		if (rc < 0)
			pr_err("[qbt] Unable to set current on %s, %d\n", name, rc);
		rc = regulator_enable(vreg);
		if (rc) {
			pr_err("[qbt] error enabling %s: %d\n", name, rc);
			regulator_put(vreg);
			vreg = NULL;
		}
		drvdata->vreg[i] = vreg;
...
static void qbt_power_onoff(struct qbt_drvdata *drvdata, int power_onoff)
{
	pr_debug("[qbt] %s   power_onoff = %d \n", __func__, power_onoff);
	if (power_onoff) {
		vreg_setup(drvdata, "vdd_io", true);
		msleep(10);
//		vreg_setup(drvdata, "vcc_spi", true);
		vreg_setup(drvdata, "vdd_ana", true);
		drvdata->prepared = 1;
		msleep(10);
	} else {
		vreg_setup(drvdata, "vdd_io", false);
//		vreg_setup(drvdata, "vcc_spi", false);
		vreg_setup(drvdata, "vdd_ana", false);
		drvdata->prepared = 0;
		msleep(10);
	}
}
...
{
	struct qbt_drvdata *drvdata = dev_get_drvdata(dev);

	pr_debug("[S]%s count=%d\n", __func__, (int)count);
	if (!strncmp(buf, "enable", strlen("enable"))) {
		qbt_power_onoff(drvdata, 1);
	} else if (!strncmp(buf, "disable", strlen("disable"))) {
		qbt_power_onoff(drvdata, 0);
	} else {
		pr_err("[E]%s -EINVAL\n", __func__);
		return -EINVAL;
	}
	...
	unsigned int type, unsigned int code, int value)
{
	...
	if (report_event) {
		if (!fd_touch->config.touch_fd_enable || !drvdata->fd_gpio.irq_enabled) {
			memcpy(fd_touch->last_events, fd_touch->current_events, MT_MAX_FINGERS *sizeof(struct touch_event));
		} else {
...
static const struct input_device_id qbt_touch_ids[] = {
	{
		...
		.evbit = {BIT_MASK(EV_ABS) | BIT_MASK(EV_REL)},
	},
	{},
};
...
/* SHARP_EXTEND [FINGERPRINT-Drv-068] Start */
		if (current_event.id < 0 && last_event.id < 0 && !td) {
			if (slot > 0) {
				pr_debug("skip to report dummy finger down\n");
				break;
			}
			finger_event.state = QBT_EVENT_FINGER_DOWN;
			td = true;
			pm_wakeup_ws_event(wakeup_source_fp, msecs_to_jiffies(4000), true);
		}
/* SHARP_EXTEND [FINGERPRINT-Drv-068] End */
		...
		struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	case QBT_DISABLE_FD:
	{
		if (drvdata->is_wuhb_connected &&
			...
			{
				int slot;
				//pr_err(" QBT_DISABLE_FD [%d] [%d] [%d] [%d] ### clear all id", drvdata->fd_touch.last_events[0].id, drvdata->fd_touch.last_events[1].id, drvdata->fd_touch.last_events[2].id, drvdata->fd_touch.last_events[3].id );
				for (slot = 0; slot < MT_MAX_FINGERS; slot++) {
					if (drvdata->fd_touch.last_events[slot].id >= 0) {
						struct fd_event finger_event;
						struct timespec timestamp;
						finger_event.touch_valid = true;
						finger_event.state = QBT_EVENT_FINGER_UP;
						...
						finger_event.timestamp.tv_usec = timestamp.tv_nsec / 1000;
						finger_event.id = slot;
						finger_event.X = drvdata->fd_touch.last_events[slot].X;
						finger_event.Y = drvdata->fd_touch.last_events[slot].Y;
						//pr_err(" QBT_DISABLE_FD QBT_EVENT_FINGER_UP slot %d TRACKING_ID = %d\n", slot, drvdata->fd_touch.last_events[slot].id);
						qbt_fd_report_event(drvdata, &finger_event);
					}
...
/* SHARP_EXTEND [FINGERPRINT-Drv-068] Start */
	wakeup_source_fp = wakeup_source_register(dev, "qbt_wakeup");
/* SHARP_EXTEND [FINGERPRINT-Drv-068] End */
/* SHARP_EXTEND [FINGERPRINT-Drv-000] Start */
	...
	pinctrl_action(drvdata->dev, "qcom,qbt-handler", "qbt_irq_active");
/* SHARP_EXTEND [FINGERPRINT-Drv-000] End */
