 ...
 */
#ifdef CONFIG_BATTERY_SHARP
#define GET_TEMP_RETRY_CONT_MAX	5
#endif /* CONFIG_BATTERY_SHARP */
...
#ifdef CONFIG_BATTERY_SHARP
	if(ret < 0){
		dev_err(&tz->device,"ret = %d. Get temp retry process start.\n", ret);
	}
	while(ret < 0){
		mdelay(1);
		if( retry_cnt >= GET_TEMP_RETRY_CONT_MAX ){
			dev_err(&tz->device,"ret = %d, retry count max.\n", ret);
			break;
		}
		ret = tz->ops->get_temp(tz, temp);
		dev_err(&tz->device,"ret = %d, retry count = %d\n", ret, retry_cnt);
		retry_cnt++;
	}
#endif /* CONFIG_BATTERY_SHARP */
