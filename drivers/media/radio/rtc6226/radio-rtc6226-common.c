...
void rtc6226_scan(struct work_struct *work)
{
	...
	while (1) {
...
#else
		pr_info("%s valid channel %d, rssi %d threshold rssi %d\n",
#endif /* SHARP mod end */
		...
		if (radio->registers[STATUS] & STATUS_SF ||
...
#else
			pr_err("%s Seek one more time if lower freq is valid\n",
#endif /* SHARP mod end */
					__func__);
#if 0 /* SHARP mod start */
			...
			mutex_lock(&radio->lock);
			if (!time) {
				pr_err("timeout didn't receive STC for seek\n");
#endif /* SHARP mod end */
				rtc6226_q_event(radio, RTC6226_EVT_ERROR);
#if 0 /* SHARP mod start */
#else
				...
				if ((radio->recv_conf.band_low_limit *
						TUNE_STEP_SIZE) ==
							next_freq_khz &&
						rssi >= radio->rssi_th) {
					pr_err("lower band freq is valid\n");
#endif /* SHARP mod end */
...
int rtc6226_start(struct rtc6226_device *radio)
{
	...
	/* initial patch 05 */
	initbuf[0] = 0x75B4;
	retval = rtc6226_set_serial_registers(radio, initbuf, 0x9D);
	if (retval < 0)
		goto done;

	/* initial patch 06 */
	initbuf[0] = 0x6200;
	retval = rtc6226_set_serial_registers(radio, initbuf, 0x20);
	if (retval < 0)
		goto done;

	/* initial patch 07 */
	initbuf[0] = 0x0A40;
	retval = rtc6226_set_serial_registers(radio, initbuf, 0x82);
	if (retval < 0)
		goto done;
...
int rtc6226_power_up(struct rtc6226_device *radio)
{
	...
	/* mpxconfig */
	/* Disable Mute / De-emphasis / Volume 15 */
...
#else
		pr_info("Get V4L2_CONTROL V4L2_CID_PRIVATE_RSSI: RSSI = %d\n",
#endif /* SHARP mod end */
...
#else
		pr_info("Get V4L2_CID_PRIVATE_DEVICEID: DEVICEID=0x%4.4hx\n",
#endif /* SHARP mod end */
...
#define VIDIOC_PRIVATE_DQBUF _IOWR('V',BASE_VIDIOC_PRIVATE,struct v4l2_buffer)

static long rtc6226_vidioc_default(struct file *file, void *fh,
                                   bool valid_prio, unsigned int cmd, void *arg)
{
    switch( cmd ){
    case VIDIOC_PRIVATE_DQBUF:
        return rtc6226_vidioc_dqbuf( file, fh, (struct v4l2_buffer*)arg );
    default:
        break;
    }
    return -ENOIOCTLCMD;
...
const struct v4l2_ioctl_ops rtc6226_ioctl_ops = {
    ...
    .vidioc_default             =   rtc6226_vidioc_default,
};
