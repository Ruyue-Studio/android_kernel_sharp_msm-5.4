...
/* SHMDS_HUB_0138_01 add S */
#ifdef SHUB_SW_POWER_RESET
static struct regulator *shub_regulator = NULL;
static unsigned int shub_reset_enable = 0xff;
#endif
/* SHMDS_HUB_0138_01 add E */
...
#ifdef SHUB_SW_POWER_RESET
    printk("[shub]shub_set_gpio_no: %ld, %ld, %ld\n", (long)shub_gpio_no_brmp, (long)shub_gpio_no_int0, (long)shub_gpio_no_int1);
#else
...
#else
#ifdef SHUB_SW_POWER_RESET
    printk("[shub]shub_set_gpio_no: %ld, %ld, %ld\n", (long)shub_gpio_no_brmp, (long)shub_gpio_no_int0);
#else
...
int shub_sleep_clk_enable(void)
{
    ...
    pin_state = pinctrl_lookup_state(pin, "shub_sleep_clk");
    if(pin_state == NULL){
        printk("[shub]%s: Null Error", __FUNCTION__);
        return -1;
    }
    ret = pinctrl_select_state(pin, pin_state);
    if(ret){
        printk("[shub]%s: select call Error(ret=%d)", __FUNCTION__, ret);
        return -1;
    }
    return ret;
}
...
{
    int ret = 0;
    COM_DBG("[shub]%s(%d): begin", __FUNCTION__, enable);

    if (shub_regulator == NULL) {
        shub_regulator = devm_regulator_get(&client_mcu->dev, "shub_reset");
        if (IS_ERR(shub_regulator)){
            printk("[shub]%s(%d): devm_regulator_get Error(IS_ERR(vreg)=%d)", __FUNCTION__, enable, IS_ERR(shub_regulator));
            shub_regulator = NULL;
            return -1;
        }

        if (regulator_is_enabled(shub_regulator)) {
            COM_DBG("[shub]%s(%d): regulator is enable", __FUNCTION__, enable);
            shub_reset_enable = SHUB_RESET_H;
        } else {
            COM_DBG("[shub]%s(%d): regulator is disable", __FUNCTION__, enable);
            shub_reset_enable = SHUB_RESET_L;
        }

        if (shub_reset_enable == SHUB_RESET_H) {
            ret = regulator_enable(shub_regulator);
            if (ret != 0){
                printk("[shub]%s(%d): regulator_enable Error(ret=%d)", __FUNCTION__, enable, ret);
                regulator_put(shub_regulator);
                shub_regulator = NULL;
                return ret;
            }
        }
    ...
    if (shub_reset_enable == enable) {
        COM_DBG("[shub]%s: shub_reset_enable == enable(%d)", __FUNCTION__, enable);
        return 0;
    }

    if (enable == SHUB_RESET_H) {
        ret = regulator_enable(shub_regulator);
        if (ret != 0){
            printk("[shub]%s(%d): regulator_enable Error(ret=%d)", __FUNCTION__, enable, ret);
            return ret;
        ...
        ret = regulator_disable(shub_regulator);
        if (ret != 0){
            printk("[shub]%s(%d): regulator_disable Error(ret=%d)", __FUNCTION__, enable, ret);
            return ret;
        }
    }

    COM_DBG("[shub]%s(%d): set shub_reset_enable = %d", __FUNCTION__, enable, enable);
    shub_reset_enable = enable;
