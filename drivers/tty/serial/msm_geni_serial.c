...
static void msm_geni_serial_break_ctl(struct uart_port *uport, int ctl)
{
	...
	if (!uart_console(uport) && device_pending_suspend(uport)) {
		...
		SH_UARTLOG_DEBUG(SH_LOG_TRACE_L, uport->dev, "fail: suspended end\n");
		return;
	}
...
static int msm_geni_serial_power_on(struct uart_port *uport)
{
	...
	if (!pm_runtime_enabled(uport->dev)) {
		...
		if (ret < 0) {
			...
			SH_UARTLOG_DEBUG(SH_LOG_TRACE_M, uport->dev, "fail: suspended end(ret:%d)\n", ret);
			return ret;
		}
	}
...
static int msm_geni_serial_get_char(struct uart_port *uport)
{
	...
	if (!(msm_geni_serial_poll_bit(uport, SE_GENI_M_IRQ_STATUS,
			M_SEC_IRQ_EN, true))) {
		SH_UARTLOG_DEBUG(SH_LOG_TRACE_L, uport->dev, "fail: IRQ end(%d)\n", -ENXIO);
		return -ENXIO;
	}
	...
	if (!(msm_geni_serial_poll_bit(uport, SE_GENI_RX_FIFO_STATUS,
			RX_FIFO_WC_MSK, true))) {
		SH_UARTLOG_DEBUG(SH_LOG_TRACE_L, uport->dev, "fail: RX end(%d)\n", -ENXIO);
		return -ENXIO;
	}
...
static int msm_geni_serial_prep_dma_tx(struct uart_port *uport)
{
	...
	if (!xmit_size) {
		SH_UARTLOG_DEBUG(SH_LOG_TRACE_L, uport->dev, "fail: prep_DMA_TX end(%d)\n", -EPERM);
		return -EPERM;
	}
	...
	SH_UARTLOG_DEBUG(SH_LOG_TRACE_L, uport->dev, "end(ret:%d)\n", ret);
	return ret;
}
...
static void msm_geni_serial_start_tx(struct uart_port *uport)
{
	...
	if (msm_port->xfer_mode == FIFO_MODE) {
		...
		geni_write_reg_nolog(geni_m_irq_en, uport->membase,
							SE_GENI_M_IRQ_EN);
		SH_UARTLOG_DEBUG(SH_LOG_TRACE_H, uport->dev, "end(FIFO)\n");
		...
		SH_UARTLOG_DEBUG(SH_LOG_TRACE_H, uport->dev, "end(DMA)\n");
	}
	return;
check_flow_ctrl:
	geni_ios = geni_read_reg_nolog(uport->membase, SE_GENI_IOS);
...
static void start_rx_sequencer(struct uart_port *uport)
{
	...
	if (port->startup_in_progress) {
		SH_UARTLOG_DEBUG(SH_LOG_TRACE_H, uport->dev, "fail: startup end\n");
		return;
	}
	...
	if (!(geni_status & S_GENI_CMD_ACTIVE)) {
		IPC_LOG_MSG(port->ipc_log_misc,
			"%s: RX is Inactive, geni_sts: 0x%x\n",
						__func__, geni_status);
		SH_UARTLOG_DEBUG(SH_LOG_TRACE_H, uport->dev,
			"RX is Inactive, geni_sts: 0x%x\n",
				geni_status);
		return 0;
	}
...
exit_rx_seq:
	...
	is_rx_active = geni_status & S_GENI_CMD_ACTIVE;
	if (is_rx_active) {
		SH_UARTLOG_DEBUG(SH_LOG_TRACE_H, uport->dev, "Rx is busy(end)\n");
		return -EBUSY;
	} else {
			...
			bool drop_rx)
{
	...
	ioread32_rep((uport->membase + SE_GENI_RX_FIFOn), msm_port->rx_fifo,
								rx_fifo_wc);
	if (drop_rx) {
		SH_UARTLOG_DEBUG(SH_LOG_TRACE_L, uport->dev, "end(drop_rx)\n");
		return 0;
	}
...
exit_portsetup:
	SH_UARTLOG_DEBUG(SH_LOG_TRACE_M, uport->dev, "fail: portsetup end (ret:%d)\n", ret);
	return ret;
}
...
exit_startup:
	...
	SH_UARTLOG_DEBUG(SH_LOG_TRACE_H, uport->dev, "end (ret:%d)\n", ret);
...
static int msm_geni_serial_runtime_suspend(struct device *dev)
{
	...
	if (ret) {
		IPC_LOG_MSG(port->ipc_log_pwr, "%s: stop rx failed %d\n",
							__func__, ret);
		SH_UARTLOG_DEBUG(SH_LOG_PM, dev, "stop rx failed (ret:%d)\n", ret);
		/* Flow on from UART only for In band sleep(IBS)
		 * Avoid manual RFR FLOW ON for Out of band sleep(OBS)
		 */
		if (port->wakeup_byte && port->wakeup_irq)
...
exit_runtime_suspend:
	SH_UARTLOG_DEBUG(SH_LOG_PM, dev, "end (ret:%d)\n", ret);
	return ret;
}
