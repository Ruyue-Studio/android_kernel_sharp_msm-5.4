...
#if defined(CONFIG_SHBT_POWER_CTRL)
#include <linux/pinctrl/consumer.h>
#endif

#ifdef CONFIG_SHBT_XORST_CTRL
#include <soc/qcom/sh_smem.h>

#define REV_DVT 3
#define REV_MP 7
static unsigned int get_sh_hw_revision( void )
{
	sharp_smem_common_type *SMemCommAdrP;

	SMemCommAdrP = sh_smem_get_common_address();
	if(SMemCommAdrP == NULL) {
		pr_err("Failed to read sh_smem\n");
		return REV_MP;
	}

	return SMemCommAdrP->sh_hw_revision;
}
#endif
...
static int bt_configure_gpios(int on)
{
	...
	if (on) {
#if defined(CONFIG_SHBT_POWER_CTRL)
		if (bt_power_pdata->pins_active != NULL) {
			pinctrl_select_state(bt_power_pdata->pinctrl, bt_power_pdata->pins_active);
		}
#endif
...
#if defined(CONFIG_SHBT_POWER_CTRL)
		if (bt_power_pdata->pins_sleep != NULL) {
			pinctrl_select_state(bt_power_pdata->pinctrl, bt_power_pdata->pins_sleep);
		}
#endif
...
#if defined(CONFIG_SHBT_POWER_CTRL)
		bt_power_pdata->pinctrl = devm_pinctrl_get(&pdev->dev);
		if (IS_ERR(bt_power_pdata->pinctrl)) {
			pr_err("%s: get pinctrl failed\n", __func__);
			bt_power_pdata->pins_sleep  = NULL;
			bt_power_pdata->pins_active = NULL;
		} else {
			bt_power_pdata->pins_sleep = pinctrl_lookup_state(
									bt_power_pdata->pinctrl, "default");
			if (IS_ERR(bt_power_pdata->pins_sleep)) {
				pr_err("%s: get default pinctrl failed\n",
					__func__);
				bt_power_pdata->pins_sleep = NULL;
			} else {
				bt_power_pdata->pins_active = pinctrl_lookup_state(
										bt_power_pdata->pinctrl, "active");
				if (IS_ERR(bt_power_pdata->pins_active)) {
					pr_err("%s: get default pinctrl failed\n",
						__func__);
					bt_power_pdata->pins_active = NULL;
				}
