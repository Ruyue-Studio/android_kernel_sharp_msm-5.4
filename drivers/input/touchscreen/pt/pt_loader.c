...
#ifdef PARADE_SYSFS_BINARY_FW_UPGRADE
				 ...
				 struct pt_hex_image *row_image)
{
...
#if 1
	data = kzalloc(3 + row_image->row_size, GFP_KERNEL);
	if (!data) {
		pt_debug(dev, DL_ERROR,
			"%s: data kzalloc error\n", __func__);
		return -ENOMEM;
	}
#endif
	...
	ret_val = cmd->nonhid_cmd->prog_and_verify(dev, 0, length, data);
...
#ifdef PARADE_SYSFS_BINARY_FW_UPGRADE
static ssize_t pt_fw_data_write(struct file *filp,
		struct kobject *kobj, struct bin_attribute *bin_attr,
		char *buf, loff_t offset, size_t count)
{
	struct device *dev = container_of(kobj, struct device, kobj);
	...
	pt_debug(dev, DL_DEBUG, "%s: offset:%lld count:%zu\n",
		__func__, offset, count);

	mutex_lock(&ld->fw_lock);

	if (!ld->fw_loading) {
		mutex_unlock(&ld->fw_lock);
		return -ENODEV;
	}

	p = krealloc(ld->fw_data, offset + count, GFP_KERNEL);
	if (!p) {
		kfree(ld->fw_data);
		ld->fw_data = NULL;
		mutex_unlock(&ld->fw_lock);
	...
	memcpy(&ld->fw_data[offset], buf, count);
	ld->fw_size += count;
...
static struct bin_attribute bin_attr_fw_data = {
	.attr = {
		.name = "fw_data",
		.mode = S_IWUSR,
	},
	.size = 0,
	.write = pt_fw_data_write,
};

static ssize_t pt_fw_loading_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct pt_loader_data *ld = pt_get_loader_data(dev);
	bool fw_loading;
	...
	mutex_lock(&ld->fw_lock);
	fw_loading = ld->fw_loading;
	mutex_unlock(&ld->fw_lock);

	return sprintf(buf, "%d\n", fw_loading);
}

static ssize_t pt_fw_loading_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t size)
{
	struct pt_loader_data *ld = pt_get_loader_data(dev);
	struct pt_core_data *cd = dev_get_drvdata(dev);
...
static DEVICE_ATTR(fw_loading, S_IRUSR | S_IWUSR,
		pt_fw_loading_show, pt_fw_loading_store);
#endif
/*******************************************************************************
...
#ifdef PARADE_SYSFS_BINARY_FW_UPGRADE
		rc = device_create_file(dev, &dev_attr_fw_loading);
		if (rc) {
			pt_debug(dev, DL_ERROR,
				"%s: Error, could not create fw_loading\n",
				__func__);
			goto remove_files;
		}

		rc = device_create_bin_file(dev, &bin_attr_fw_data);
		if (rc) {
			pt_debug(dev, DL_ERROR,
				"%s: Error, could not create fw_data\n",
				__func__);
			goto remove_files;
		}
#endif
...
#ifdef SUPPORT_GKI_BUILD
	sysfs_remove_bin_file(&dev->kobj, &bin_attr_pip2_file_read);
#else
...
#ifdef PARADE_SYSFS_BINARY_FW_UPGRADE
#ifdef SUPPORT_GKI_BUILD
	sysfs_remove_bin_file(&dev->kobj, &bin_attr_fw_data);
#else
	device_remove_bin_file(dev, &bin_attr_fw_data);
#endif
	device_remove_file(dev, &dev_attr_fw_loading);
#endif
#endif /* !TTDL_KERNEL_SUBMISSION */
...
#ifdef CONFIG_TOUCHSCREEN_PARADE_MANUAL_TTCONFIG_UPGRADE
#ifdef SUPPORT_GKI_BUILD
		sysfs_remove_bin_file(&dev->kobj, &bin_attr_config_data);
#else
