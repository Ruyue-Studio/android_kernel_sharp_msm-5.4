...
#if 1
	pdata->test_mode_gpio = of_get_named_gpio(core_node, "parade,test_mode_gpio", 0);
#endif
...
#if 1
	rc = of_property_read_string(core_node, "parade,pwr-reg-name", &name);
	if (rc < 0)
		pdata->pwr_reg_name = NULL;
	else
		pdata->pwr_reg_name = name;

	rc = of_property_read_string(core_node, "parade,bus-reg-name", &name);
	if (rc < 0)
...
#if 1
	pdata->lock_power = pt_lock_power;
	pdata->check_power = pt_check_power;
#endif
