...
#include "pt_regs.h"
#include <linux/input/pt_platform.h>
...
static int pt_enable_regulator(struct pt_core_platform_data *pdata, struct device *dev, bool en)
{
	int retval;
	int rst_gpio = pdata->rst_gpio;

	mutex_lock(&pdata->regulator_lock);
	...
	if (pdata->regulator_enable == true) {
		pt_debug(dev, DL_INFO, "%s: already regulator is enabled\n",
			__func__);
		retval = 0;
		goto exit;
	}
...
disable_pwr_reg:
	if (pdata->regulator_enable == false) {
		pt_debug(dev, DL_INFO, "%s: already regulator is disabled\n",
			__func__);
		retval = 0;
		goto exit;
	}
...
exit:
	mutex_unlock(&pdata->regulator_lock);
	return retval;
}

static int pt_get_regulator(struct pt_core_platform_data *pdata, struct device *dev, bool get)
{
	int retval;

	pt_debug(dev, DL_INFO,
		"%s: %s start\n",
		__func__, (get ? "get" : "put"));
	...
	if (pdata->pwr_reg_name != NULL && *pdata->pwr_reg_name != 0) {
		pdata->pwr_reg = regulator_get(dev,
				pdata->pwr_reg_name);
		if (IS_ERR(pdata->pwr_reg)) {
			...
			pt_debug(dev, DL_INFO,
				"%s: success to get power regulator\n",
				__func__);
		}
	}
	else {
		pt_debug(dev, DL_ERROR,
			"%s: Failed to get power regulator from dtsi\n",
			__func__);
	}

	if (pdata->bus_reg_name != NULL && *pdata->bus_reg_name != 0) {
		pdata->bus_reg = regulator_get(dev,
				pdata->bus_reg_name);
		if (IS_ERR(pdata->bus_reg)) {
			...
			pt_debug(dev, DL_INFO,
				"%s: success to get bus regulator\n",
				__func__);
		}
	}
	else {
		pt_debug(dev, DL_ERROR,
			"%s: Failed to get bus regulator from dtsi\n",
			__func__);
	}
		...
		int on, struct device *dev)
{
	...
	if (pdata->pinctrl == NULL) {
		...
		if (IS_ERR_OR_NULL(pdata->pinctrl)) {
			...
			if (IS_ERR_OR_NULL(pdata->tpin_state_suspend)) {
				pt_debug(dev, DL_ERROR, "pinctrl lookup failed for test_mode_pull_down\n");
			...
			if (IS_ERR_OR_NULL(pdata->int_state_active)) {
				pt_debug(dev, DL_ERROR, "pinctrl lookup failed for int_active\n");
			...
			if (IS_ERR_OR_NULL(pdata->int_state_standby)) {
				pt_debug(dev, DL_ERROR, "pinctrl lookup failed for int_standby\n");
			...
			if (IS_ERR_OR_NULL(pdata->int_state_no_pull)) {
				pt_debug(dev, DL_ERROR, "pinctrl lookup failed for int_no_pull\n");
			...
			if (IS_ERR_OR_NULL(pdata->reset_state_active)) {
				pt_debug(dev, DL_ERROR, "pinctrl lookup failed for reset_active\n");
			...
			if (IS_ERR_OR_NULL(pdata->reset_state_standby)) {
				pt_debug(dev, DL_ERROR, "pinctrl lookup failed for reset_standby\n");
	...
	pt_debug(dev, DL_INFO,
		"%s: %s end - Configured IRQ GPIO %d, TP_XRES GPIO %d\n",
		__func__, (on ? "On" : "Off"), irq_gpio, rst_gpio);
	return rc;
#else
	int rst_gpio     = pdata->rst_gpio;
	int irq_gpio     = pdata->irq_gpio;
...
{
#if 1
	pt_enable_regulator(pdata, dev, true);
...
{
#if 1
	pt_enable_regulator(pdata, dev, false);
		...
		struct device *dev, pt_platform_read read)
{
	...
	while (retry--) {
		...
		pt_debug(dev, DL_ERROR, "%s: Performing a reset\n",
			__func__);
		...
		struct device *dev)
{
	...
	if (on == PT_MT_POWER_ON) {
		if (pdata->pwr_reg == NULL || pdata->bus_reg == NULL) {
			rc = pt_get_regulator(pdata, dev, true);
			if (rc < 0) {
				return -1;
			}
		}
		rc = pt_power(pdata, 1, dev, 0);
	} else {
		pt_power(pdata, 0, dev, 0);

		pt_get_regulator(pdata, dev, false);
	}
	...
	struct device *dev)
{
	...
	if (on == PT_MT_IRQ_REG) {
		...
		if (pdata->irq_stat) {
			/* Initialize IRQ */
			pt_debug(dev, DL_INFO, "%s: Value Passed to gpio_to_irq =%d\n",
...
int pt_lock_power(struct pt_core_platform_data *pdata, int req,
		struct device *dev)
{
	if (req == 0) {
		mutex_unlock(&pdata->regulator_lock);
...
int pt_check_power(struct pt_core_platform_data *pdata,
		struct device *dev)
{
	if (pdata->regulator_enable == true) {
		return 1;
