...
#ifdef PARADE_SEND_SHTERM_EVENT_ENABLE
	#include <misc/shterm_k.h>
#endif /* PARADE_SEND_SHTERM_EVENT_ENABLE */

static int WAKEUP_WAIT_MS = 50;
#ifdef	PT_DEVELOP_MODE_ENABLE
	module_param(WAKEUP_WAIT_MS, int, S_IRUGO | S_IWUSR);
#endif
...
#if defined(PARADE_LPMODE_ENABLE)
#define PARADE_LPMODE_TYPE_NON_CONTINUOUS	0
#define PARADE_LPMODE_TYPE_CONTINUOUS		1

#define PARADE_LPMODE_REQ_NONE				0x00
#define PARADE_LPMODE_REQ_COMMON			0x01
#define PARADE_LPMODE_REQ_ECO				0x02

static void pt_set_lpmode_init(struct pt_core_data *cd);
static void pt_lpmode_set_state(struct pt_core_data *cd, int type, int req, int on);
#endif /* PARADE_LPMODE_ENABLE */
#if defined(PARADE_GLOVE_ENABLE)
static int pt_set_glove_detect_enable(struct pt_core_data *cd);
static int pt_set_glove_detect_disable(struct pt_core_data *cd);
static int pt_set_glove_state(struct pt_core_data *cd, int on);
static int pt_set_glove_detect_init(struct pt_core_data *cd);
#endif /* PARADE_GLOVE_ENABLE */
#if defined(PARADE_CTRL_FW_REPORT_RATE_ENABLE)
static int pt_set_fw_report_rate(struct pt_core_data *cd, int on);
static int pt_set_fw_report_rate_state(struct pt_core_data *cd, int enable);
static int pt_set_fw_report_rate_init(struct pt_core_data *cd);
#endif /* PARADE_CTRL_FW_REPORT_RATE_ENABLE */
#if defined(PARADE_LPWG_ENABLE)
static void pt_set_lpwg_on(struct pt_core_data *cd);
static void pt_set_lpwg_off(struct pt_core_data *cd);
static u8 pt_get_lpwg_state(struct pt_core_data *cd);
static int pt_lpwg_set_state(struct pt_core_data *cd, int gesture, int on);
#endif /* PARADE_LPWG_ENABLE */
#if defined(PARADE_CHARGER_ARMOR_ENABLE)
static void pt_set_charger_armor_on(struct pt_core_data *cd);
static void pt_set_charger_armor_off(struct pt_core_data *cd);
static int pt_set_charger_armor_state(struct pt_core_data *cd, int on);
static void pt_set_charger_armor_init(struct pt_core_data *cd);
#endif /* PARADE_CHARGER_ARMOR_ENABLE */
#if defined(PARADE_FINGERPRINT_NOTIFY_ENABLE)
static void pt_fingerprint_notify_init(struct pt_core_data *cd);
#endif /* PARADE_FINGERPRINT_NOTIFY_ENABLE */
#if defined(PARADE_GRIP_SUPPRESSION_ENABLE)
static void pt_set_grip_suppression_init(struct pt_core_data *cd);
#endif /* PARADE_GRIP_SUPPRESSION_ENABLE */
#if defined(PARADE_CTRL_FOS_ENABLE_PARAM_ENABLE)
static void pt_ctrl_fos_enable_param_enable(struct pt_core_data *cd);
static void pt_ctrl_fos_enable_param_disable(struct pt_core_data *cd);
#endif /* PARADE_CTRL_FOS_ENABLE_PARAM_ENABLE */
#if defined(PARADE_SUPPORT_DEGRADE_FLG_ENABLE)
static int pt_check_degrade_flg_(struct pt_core_data *cd);
...
static void pt_pip2_cmd_calculate_crc(struct pip2_cmd_structure *cmd,
	u8 extra_bytes)
{
#ifdef MEMORY_COPY_CHECK_ENABLE
	u8 *buf;
	u8 buf_local[PT_MAX_PIP2_MSG_SIZE + 1] = {0};
	int alloc_size = 0;
	u8 *alloc_buf;
#else
	u8 buf[PT_MAX_PIP2_MSG_SIZE + 1] = {0};
#endif /* MEMORY_COPY_CHECK_ENABLE */
	unsigned short crc;

#ifdef MEMORY_COPY_CHECK_ENABLE
	alloc_size = sizeof(u8) * (4 + (cmd->len - extra_bytes));
	if (sizeof(buf_local) < alloc_size) {
		alloc_buf = (u8 *)kzalloc(alloc_size, GFP_KERNEL);
		if(alloc_buf == NULL){
			pr_err("%s: alloc error. size = %d\n", __func__, alloc_size);
			return;
		}
		buf = alloc_buf;
	}
...
again:
	...
	if (read_len && data) {
		...
		if (read_len > read_length) {
			pt_debug(cd->dev, DL_ERROR,
					"%s: read_len error. (read_len=%d read_length=%d)\n", __func__, read_len, read_length);
			return -ENOMEM;
		}
#endif /* MEMORY_COPY_CHECK_ENABLE */
		memcpy(&data[off_buf], &cd->response_buf[10], read_len);
...
#if defined(PARADE_SUPPORT_DEGRADE_FLG_ENABLE)
/*******************************************************************************
 * FUNCTION: pt_pip_get_noise_metrics_
 *
 * SUMMARY: Sends a PIP command 0x07 Get Noise Metrics to the DUT and returns
 *  a metric associated with the level of noise.
 *
 * RETURN:
 *   0 = success
 *  !0 = failure
 ...
 * PARAMETERS:
 *  *cd             - pointer to core data
 *  *return_buf     - pointer to store the return data
 *  *return_buf_size- return data buffer size
 ******************************************************************************/
static int pt_pip_get_noise_metrics_(struct pt_core_data *cd,
		u8 *return_buf, int return_buf_size)
{
	int data_len;
	int rc = 0;
	struct pt_hid_output hid_output = {
		CREATE_PIP1_FW_CMD(PIP1_CMD_ID_GET_NOISE_METRICS),
	};

	rc = pt_hid_send_output_and_wait_(cd, &hid_output);
	if (rc) {
		pt_debug(cd->dev, DL_ERROR,
			"%s: Fail get_noise_metrics rc=%d\n", __func__, rc);
		return rc;
	}

	data_len = get_unaligned_le16(&cd->input_buf[0]) - 5;
	if (data_len <= 0 || data_len > return_buf_size) {
		pt_debug(cd->dev, DL_ERROR,
			"%s: get_noise_metrics size check error. data_len=%d buf_size=%d\n", __func__, data_len, return_buf_size);
		return -EPROTO;
	}

	memcpy(return_buf, &cd->response_buf[5], data_len);
...
#if defined(PARADE_SUPPORT_DEGRADE_FLG_ENABLE)
	{0x02, 0x01, 0x00, 0x02,    0x06, 0x01},	/* RX NOT FLAT */
#endif /* PARADE_SUPPORT_DEGRADE_FLG_ENABLE */
	{0x03, 0x01, 0x00, 0x08,    0x00, 0x01},	/* NOISE EFFECT */
#if defined(PARADE_SUPPORT_DEGRADE_FLG_ENABLE)
	{0x03, 0x01, 0x00, 0x02,    0x03, 0x01},	/* BL_INIT */
	{0x03, 0x01, 0x00, 0x02,    0x04, 0x01},	/* DEGDRY */
	{0x03, 0x01, 0x00, 0x02,    0x05, 0x01},	/* DEGMOIST */
#endif /* PARADE_SUPPORT_DEGRADE_FLG_ENABLE */
...
static int pt_put_device_into_easy_wakeup_(struct pt_core_data *cd)
{
	...
	rc = pt_hid_output_initialize_baselines_(cd, PT_IB_SM_MUTCAP |
...
#if defined(PARADE_LPMODE_ENABLE)
	cd->lpmode_req_state = PARADE_LPMODE_REQ_NONE;
#endif /* PARADE_LPMODE_ENABLE */
#if defined(PARADE_GLOVE_ENABLE)
	if(cd->glove_enable_state != 0){
		pt_set_glove_detect_disable(cd);
	}
#endif /* PARADE_GLOVE_ENABLE */
#if defined(PARADE_CTRL_FW_REPORT_RATE_ENABLE)
	pt_set_fw_report_rate(cd, 0);
#endif /* PARADE_CTRL_FW_REPORT_RATE_ENABLE */
#if defined(PARADE_LPWG_ENABLE)
	if (cd->lpwg_req_state != 0) {
		pt_set_lpwg_on(cd);
	}
	cd->gesture_id = 0;
#endif /* PARADE_LPWG_ENABLE */
#if defined(PARADE_CTRL_FOS_ENABLE_PARAM_ENABLE)
	pt_ctrl_fos_enable_param_disable(cd);
#endif /* PARADE_CTRL_FOS_ENABLE_PARAM_ENABLE */
...
static int parse_command_input(struct pt_core_data *cd, int size)
{
	...
	if (size > PT_MAX_INPUT) {
		pt_debug(cd->dev, DL_ERROR,
			"%s: cd->response_buf memcpy size over. (%d > %d)\n",
			__func__, size, PT_MAX_INPUT);
		return -ENOMEM;
	}
#endif /* MEMORY_COPY_CHECK_ENABLE */
	...
	if (IS_EASY_WAKE_CONFIGURED(cd->easy_wakeup_gesture)) {
		if (cd->sleep_state == SS_SLEEP_ON) {
			...
			if (!dev->power.is_suspended) {
				pt_debug(dev, DL_DEBUG, "%s: dev->power.is_suspended=false\n",
					__func__);
				goto read;
			}
			pt_debug(dev, DL_DEBUG, "%s: wait_event_timeout start\n",
				__func__);
			t = wait_event_timeout(cd->wait_q,
					(cd->lpwg_system_suspended == false),
					msecs_to_jiffies(2000));
			if (IS_TMO(t)) {
				pt_debug(dev, DL_DEBUG, "%s: wait_event_timeout timeout end\n",
					__func__);
				return -EAGAIN;
			}
			pt_debug(dev, DL_DEBUG, "%s: wait_event_timeout end\n",
				__func__);
#else
			if (!dev->power.is_suspended)
				goto read;
...
static int pt_core_wake_(struct pt_core_data *cd)
{
	...
	if (!(cd->cpdata->flags & PT_CORE_FLAG_SKIP_RESUME)) {
		if (IS_EASY_WAKE_CONFIGURED(cd->easy_wakeup_gesture)) {
#if defined(PARADE_LPWG_ENABLE)
			if (PARADE_PRM_LPWG_RESUME_RESET_ENABLE != 0) {
				if (cd->gesture_id == GESTURE_SINGLE_FINGER_LONG_TOUCH) {
...
#if 1
	if (WAKEUP_WAIT_MS != 0) {
		usleep_range(WAKEUP_WAIT_MS * 1000, (WAKEUP_WAIT_MS + 1) * 1000);
	}
#endif
#if defined(PARADE_CTRL_FOS_ENABLE_PARAM_ENABLE)
	pt_ctrl_fos_enable_param_enable(cd);
#endif /* PARADE_CTRL_FOS_ENABLE_PARAM_ENABLE */
...
#if defined(PARADE_LPMODE_ENABLE)
	pt_set_lpmode_init(cd);
#endif /* PARADE_LPMODE_ENABLE */
#if defined(PARADE_LPWG_ENABLE)
	mutex_lock(&cd->lpwg_wake_lock_on_mutex);

	if (cd->lpwg_wake_lock_on == true) {
		cd->lpwg_wake_lock_on = false;
		cancel_delayed_work(&cd->lpwg_wake_lock_timer);
		pt_debug(cd->dev, DL_DEBUG, "%s: wake_unlock(lpwg_wake_lock)\n",
			__func__);
		wake_unlock(&cd->lpwg_wake_lock);
	}

	mutex_unlock(&cd->lpwg_wake_lock_on_mutex);

	if (cd->lpwg_cur_state != 0) {
		pt_set_lpwg_off(cd);
	}
#endif /* PARADE_LPWG_ENABLE */
#if defined(PARADE_GLOVE_ENABLE)
	pt_set_glove_detect_init(cd);
#endif /* PARADE_GLOVE_ENABLE */
#if defined(PARADE_CTRL_FW_REPORT_RATE_ENABLE)
	pt_set_fw_report_rate_init(cd);
#endif /* PARADE_CTRL_FW_REPORT_RATE_ENABLE */
#if defined(PARADE_CHARGER_ARMOR_ENABLE)
	pt_set_charger_armor_init(cd);
#endif /* PARADE_CHARGER_ARMOR_ENABLE */
#if defined(PARADE_FINGERPRINT_NOTIFY_ENABLE)
	pt_fingerprint_notify_init(cd);
#endif /* PARADE_FINGERPRINT_NOTIFY_ENABLE */
#if defined(PARADE_GRIP_SUPPRESSION_ENABLE)
	pt_set_grip_suppression_init(cd);
#endif /* PARADE_GRIP_SUPPRESSION_ENABLE */
#if defined(PARADE_SUPPORT_DEGRADE_FLG_ENABLE)
	pt_check_degrade_flg_(cd);
#endif /* PARADE_SUPPORT_DEGRADE_FLG_ENABLE */
...
static int pt_core_suspend_(struct device *dev)
{
	...
	if (IS_EASY_WAKE_CONFIGURED(cd->easy_wakeup_gesture)) {
		pt_debug(dev, DL_DEBUG, "%s: set cd->lpwg_system_suspended = true\n",
			__func__);

		cd->lpwg_system_suspended = true;
	}
#endif /* PARADE_LPWG_ENABLE */
...
static int pt_core_resume_(struct device *dev)
{
	...
	if (IS_EASY_WAKE_CONFIGURED(cd->easy_wakeup_gesture)) {
		pt_debug(cd->dev, DL_DEBUG, "%s: set cd->lpwg_system_suspended = false\n",
			__func__);

		cd->lpwg_system_suspended = false;

		pt_debug(cd->dev, DL_DEBUG, "%s: wake_up(&cd->wait_q)\n",
			__func__);
...
#if 1
static DEFINE_MUTEX(module_init_lock);
#endif
...
#if defined(PARADE_ASYNC_WAKE_ENABLE)
static void pt_async_wake_work(struct work_struct *work)
{
	struct pt_core_data *cd =
			container_of(work, struct pt_core_data,
					async_wake_work);
	...
	cd->async_wake_flg = false;
	wake_up(&cd->async_wake_wait_q);

	pt_debug(cd->dev, DL_INFO, "%s: end.\n", __func__);
}
#endif /* PARADE_ASYNC_WAKE_ENABLE */

#if defined(PARADE_SUPPORT_DEGRADE_FLG_ENABLE)
static int pt_check_degrade_flg_(struct pt_core_data *cd)
{
	u8 return_buf[50];
	int return_buf_size = sizeof(return_buf);
	int rc;
	u8 status = 0;
	...
	rc = pt_pip_get_noise_metrics_(cd, &return_buf[0], return_buf_size);
	if (rc != 0) {
		pt_debug(cd->dev, DL_ERROR,
			"%s: get_noise_metrics error. rc=%d\n", __func__, rc);
		return -1;
	}

	if ((return_buf[16] & 0x02) || (return_buf[16] & 0x04)) {
		if((md != NULL) && (md->si != NULL)) {
			if (md->fw_num_prv_rec == 0) {
				pt_debug(cd->dev, DL_INFO, "%s: initialize_baselines by%s%s.\n",
					__func__,
					(return_buf[16] & 0x02 ? " DegDry" : ""),
					(return_buf[16] & 0x04 ? " DegMoist" : "")
					);
				...
				pt_debug(cd->dev, DL_INFO, "%s: Skip initialize baselines because fw_num_prv_rec=%d\n",
					__func__, md->fw_num_prv_rec);

				md->degrade_flg_info.request_touchup_initialize_baselines_flg = true;
			}
		}
	}
 ...
 ******************************************************************************/
#if defined(PARADE_LPMODE_ENABLE)
static void pt_set_lpmode_on(struct pt_core_data *cd)
{
	int retval = 0;

	retval |= pt_pip_set_param_(cd, 0x1C, 0x0064, 2);
	retval |= pt_pip_set_param_(cd, 0x06, 0x01, 1);
	if (retval != 0) {
		pt_debug(cd->dev, DL_ERROR, "%s: Failed to set doze mode ON\n",
				__func__);
	}
}
static void pt_set_lpmode_off(struct pt_core_data *cd)
{
	int retval = 0;

	retval |= pt_pip_set_param_(cd, 0x06, 0x00, 1);
	if (retval != 0) {
		pt_debug(cd->dev, DL_ERROR, "%s: Failed to set doze mode OFF\n",
				__func__);
	}
}
static u8 pt_get_lpmode_state(struct pt_core_data *cd)
{
	u8 req = (cd->lpmode_req_state | cd->lpmode_continuous_req_state);

	if(req == PARADE_LPMODE_REQ_NONE){
		return 1;
	}
	return 0;
}
static void pt_set_lpmode_init(struct pt_core_data *cd)
{
	if(pt_get_lpmode_state(cd) != 0){
		pt_set_lpmode_off(cd);
	}else{
		pt_set_lpmode_on(cd);
	}
}
static void pt_lpmode_set_state(struct pt_core_data *cd, int type, int req, int on)
{
	...
	if(on){
		if(type == PARADE_LPMODE_TYPE_NON_CONTINUOUS){
			if((cd->lpmode_req_state & req) == 0){
				cd->lpmode_req_state |= req;
				changed = 1;
				pt_debug(cd->dev, DL_DEBUG, "%s: [LPMODE]<ON> type = NON CONTINUOUS, req = %s\n",
										__func__,
										(req == PARADE_LPMODE_REQ_COMMON)? "common" :
										(req == PARADE_LPMODE_REQ_ECO)? "eco" : "unknown");
			}
		}else{
			if((cd->lpmode_continuous_req_state & req) == 0){
				cd->lpmode_continuous_req_state |= req;
				changed = 1;
				pt_debug(cd->dev, DL_DEBUG, "%s: [LPMODE]<ON> type = CONTINUOUS, req = %s\n",
										__func__,
										(req == PARADE_LPMODE_REQ_COMMON)? "common" :
										(req == PARADE_LPMODE_REQ_ECO)? "eco" : "unknown");
			}
		...
		if(type == PARADE_LPMODE_TYPE_NON_CONTINUOUS){
			if((cd->lpmode_req_state & req) != 0){
				cd->lpmode_req_state &= ~req;
				changed = 1;
				pt_debug(cd->dev, DL_DEBUG, "%s: [LPMODE]<OFF> type = NON CONTINUOUS, req = %s\n",
										__func__,
										(req == PARADE_LPMODE_REQ_COMMON)? "common" :
										(req == PARADE_LPMODE_REQ_ECO)? "eco" : "unknown");
			}
			...
			if((cd->lpmode_continuous_req_state & req) != 0){
				cd->lpmode_continuous_req_state &= ~req;
				changed = 1;
				pt_debug(cd->dev, DL_DEBUG, "%s: [LPMODE]<OFF> type = CONTINUOUS, req = %s\n",
										__func__,
										(req == PARADE_LPMODE_REQ_COMMON)? "common" :
										(req == PARADE_LPMODE_REQ_ECO)? "eco" : "unknown");
			}
	...
	pt_debug(cd->dev, DL_DEBUG,
						"%s: [LPMODE]lpmode request recieved. req_state = 0x%02x, continuous_req_state = 0x%02x\n",
						__func__,
						cd->lpmode_req_state, cd->lpmode_continuous_req_state);
...
#if defined(PARADE_GLOVE_ENABLE)
static int pt_fwctl_glove_enable(struct pt_core_data *cd)
{
	int retval;

	retval = pt_pip_set_param_(cd, 0x02, 0x03, 1);
	if (retval != 0) {
		pt_debug(cd->dev, DL_ERROR,
				"Failed to set glove mode ON");
	}
	if (PARADE_GLOVE_GRIP_ENABLE != 0) {
		pt_pip_set_param_(cd, 0x0E, PARADE_GLOVE_GRIP_XEDG, 2);
		pt_pip_set_param_(cd, 0x0F, PARADE_GLOVE_GRIP_XEDG, 2);
		pt_pip_set_param_(cd, 0x16, 0, 1);
		pt_pip_set_param_(cd, 0x18, 1, 1);
	}

	return retval;
}
static int pt_fwctl_glove_disable(struct pt_core_data *cd)
{
	int retval;

	retval = pt_pip_set_param_(cd, 0x02, 0x01, 1);
	if (retval != 0) {
		pt_debug(cd->dev, DL_ERROR,
				"Failed to set glove mode OFF");
	}
	if (PARADE_GLOVE_GRIP_ENABLE != 0) {
		pt_pip_set_param_(cd, 0x18, 0, 1);
	}
...
static int pt_set_glove_detect_enable(struct pt_core_data *cd)
{
	...
	if(req == 1){
		rc = pt_fwctl_glove_enable(cd);

		#if defined(PARADE_CTRL_FW_REPORT_RATE_ENABLE)
			if(cd->fw_report_rate_req_state != 0){
				pt_set_fw_report_rate(cd, 0);
			}
		#endif /* PARADE_CTRL_FW_REPORT_RATE_ENABLE */
...
static int pt_set_glove_detect_disable(struct pt_core_data *cd)
{
	...
	rc = pt_fwctl_glove_disable(cd);

	#if defined(PARADE_CTRL_FW_REPORT_RATE_ENABLE)
		if(cd->fw_report_rate_req_state != 0){
			pt_set_fw_report_rate(cd, 1);
		}
	#endif /* PARADE_CTRL_FW_REPORT_RATE_ENABLE */
...
static int pt_set_glove_state(struct pt_core_data *cd, int on)
{
	...
	if(on){
		if(cd->glove_enable_state == 0){
			cd->glove_enable_state = 1;

			if (cd->sleep_state == SS_SLEEP_OFF) {
				pt_set_glove_detect_enable(cd);
			}
		}
	}
	else{
		if(cd->glove_enable_state != 0){
			cd->glove_enable_state = 0;
...
static int pt_set_glove_detect_init(struct pt_core_data *cd)
{
	...
	if(cd->glove_enable_state == 0){
		rc = pt_set_glove_detect_disable(cd);
	}else{
		rc = pt_set_glove_detect_enable(cd);
	}
...
#if defined(PARADE_CTRL_FW_REPORT_RATE_ENABLE)
static int pt_set_fw_report_rate_get_report_table_id(struct pt_core_data *cd, int report_rate)
{
	int i;
	int table_id = PT_CTRL_FW_REPORT_RATE_TABLE_SIZE - 1;

	if (report_rate == 0) {
		report_rate = PARADE_CTRL_FW_REPORT_RATE_NORMAL;
	}
#if 0
	else if (report_rate == 1) {
		report_rate = PARADE_CTRL_FW_REPORT_RATE_HIGH;
	}
#endif

	for (i = 0; i < PT_CTRL_FW_REPORT_RATE_TABLE_SIZE; i++) {
		if (pt_ctrl_fw_report_rate_table[i].report_rate >= report_rate) {
			table_id = i;
			break;
		}
	}
...
static int pt_fwctl_set_fw_report_rate(struct pt_core_data *cd, unsigned short value)
{
	int retval;

	retval = pt_pip_set_param_(cd, 0x1B, value, 1);
	if (retval != 0) {
		pt_debug(cd->dev, DL_ERROR,
				"Failed to set report rate");
	}

	return retval;
}
static int pt_set_fw_report_rate(struct pt_core_data *cd, int enable)
{
	int rc;
	unsigned short value;
	int table_id;
	int report_rate = cd->fw_report_rate_req_state;
	...
	table_id = pt_set_fw_report_rate_get_report_table_id(cd, report_rate);
	value = pt_ctrl_fw_report_rate_table[table_id].set_value;

	rc = pt_fwctl_set_fw_report_rate(cd, value);

	return rc;
}

static int pt_set_fw_report_rate_state(struct pt_core_data *cd, int report_rate)
{
	int rc;
	...
	if(cd->fw_report_rate_req_state != report_rate){
		cd->fw_report_rate_req_state = report_rate;
...
#if defined(PARADE_LPWG_ENABLE)
static void pt_set_finger_detect_enable(struct pt_core_data *cd)
{
	int retval;
	...
	if (retval != 0) {
		pt_debug(cd->dev, DL_ERROR,
				"Failed to set finger_detect ON");
	}
}
static void pt_set_finger_detect_disable(struct pt_core_data *cd)
{
	int retval;

	retval = pt_pip_set_param_(cd, 0x02, 0x00, 1);
	if (retval != 0) {
		pt_debug(cd->dev, DL_ERROR,
				"Failed to set finger_detect OFF");
	}
}
static void pt_set_lpwg_on(struct pt_core_data *cd)
{
	...
	retval = 0;
	cd->easy_wakeup_gesture = cd->lpwg_req_state;
	if (retval != 0) {
		pt_debug(cd->dev, DL_ERROR, "%s: Failed to set lpwg mode ON\n", __func__);
	}
	else{
		cd->lpwg_cur_state = cd->lpwg_req_state;
	}
}
static void pt_set_lpwg_off(struct pt_core_data *cd)
{
	...
	retval = 0;
	cd->easy_wakeup_gesture = 0;
	if (retval != 0) {
		pt_debug(cd->dev, DL_ERROR, "%s: Failed to set lpwg mode OFF\n", __func__);
	}
	else{
		cd->lpwg_cur_state = 0;
	}
...
static int pt_lpwg_set_state(struct pt_core_data *cd, int gesture, int on)
{
	...
	if(on){
		if((cd->lpwg_req_state & gesture) == 0){
			cd->lpwg_req_state |= gesture;
			changed = 1;
			pt_debug(cd->dev, DL_DEBUG, "%s: [lpwg] 0x%02X <ON>\n",
				__func__, gesture);
		}
	}
	else{
		if((cd->lpwg_req_state & gesture) != 0){
			cd->lpwg_req_state &= ~gesture;
			changed = 1;
			pt_debug(cd->dev, DL_DEBUG, "%s: [lpwg] 0x%02X <OFF>\n",
				__func__, gesture);
		}
	}
	...
	if(changed){
		if (cd->sleep_state == SS_SLEEP_ON) {
			if(pt_get_lpwg_state(cd) == 0 && cd->lpwg_req_state != 0){
				pt_core_wake_(cd);
				pt_core_sleep_(cd);
			}
			else if(pt_get_lpwg_state(cd) != 0 && cd->lpwg_req_state == 0){
				pt_set_lpwg_off(cd);
				pt_core_poweroff_device_(cd);
			}
			else{
				pt_set_lpwg_on(cd);
				pt_set_finger_detect_disable(cd);
				pt_core_wake_device_from_deep_sleep_(cd);
				pt_put_device_into_easy_wakeup_(cd);
				pt_set_finger_detect_enable(cd);
			}
		}
	}

	pt_debug(cd->dev, DL_DEBUG, "%s: [lpwg]lpwg request recieved. req_state = 0x%02x\n",
		__func__, cd->lpwg_req_state);
...
static void pt_lpwg_wake_lock_timer(struct work_struct *work)
{
	struct delayed_work *dw = container_of(work, struct delayed_work, work);
	struct pt_core_data *cd = container_of(dw, struct pt_core_data, lpwg_wake_lock_timer);
...
static void pt_set_charger_armor_on(struct pt_core_data *cd)
{
	...
	retval = pt_pip_set_param_(cd, 0x04, 0x01, 1);
	if (retval != 0) {
		pt_debug(cd->dev, DL_ERROR,
...
static void pt_set_charger_armor_off(struct pt_core_data *cd)
{
	...
	retval = pt_pip_set_param_(cd, 0x04, 0x00, 1);
	if (retval != 0) {
		pt_debug(cd->dev, DL_ERROR,
...
static int pt_set_charger_armor_state(struct pt_core_data *cd, int on)
{
	...
	if(on){
		if(cd->charger_armor_req_state == 0){
			cd->charger_armor_req_state = 1;
			pt_debug(cd->dev, DL_DEBUG, "%s: [charger_armor]<ON>\n",
				__func__);

			if (cd->sleep_state == SS_SLEEP_OFF) {
				pt_set_charger_armor_on(cd);
			}
		}
	}
	else{
		if(cd->charger_armor_req_state != 0){
			cd->charger_armor_req_state = 0;
			pt_debug(cd->dev, DL_DEBUG, "%s: [charger_armor]<OFF>\n",
				__func__);

			if (cd->sleep_state == SS_SLEEP_OFF) {
				pt_set_charger_armor_off(cd);
			}
		}
	}
...
#if defined(PARADE_FINGERPRINT_NOTIFY_ENABLE)
static void pt_fingerprint_notify_on(struct pt_core_data *cd)
{
	int retval;

	retval = pt_pip_set_param_(cd, 0x30, 0x01, 1);
	if (retval != 0) {
		pt_debug(cd->dev, DL_ERROR,
				"Failed to set fingerprint_notify ON");
	}
}
static void pt_fingerprint_notify_off(struct pt_core_data *cd)
{
	int retval;

	retval = pt_pip_set_param_(cd, 0x30, 0x00, 1);
	if (retval != 0) {
		pt_debug(cd->dev, DL_ERROR,
				"Failed to set fingerprint_notify OFF");
	}
}
static void pt_fingerprint_notify_set_test(struct pt_core_data *cd, int high)
{
	int retval;

	if(high == 1) {
		retval = pt_pip_set_param_(cd, 0x30, 0x02, 1);
	}
	else {
		retval = pt_pip_set_param_(cd, 0x30, 0x03, 1);
	}
	if (retval != 0) {
		pt_debug(cd->dev, DL_ERROR,
				"Failed to set fingerprint_notify test");
	}
}
static int pt_fingerprint_notify_set_state(struct pt_core_data *cd, int on)
{
	int rc;
	...
	if(on){
		if(cd->fingerprint_notify_req_state == 0){
			cd->fingerprint_notify_req_state = 1;
			pt_debug(cd->dev, DL_DEBUG, "%s: [fingerprint_notify]<ON>\n",
				__func__);

			if (cd->sleep_state == SS_SLEEP_OFF) {
				pt_fingerprint_notify_on(cd);
			}
		}
	}
	else{
		if(cd->fingerprint_notify_req_state != 0){
			cd->fingerprint_notify_req_state = 0;
			pt_debug(cd->dev, DL_DEBUG, "%s: [fingerprint_notify]<OFF>\n",
				__func__);

			if (cd->sleep_state == SS_SLEEP_OFF) {
				pt_fingerprint_notify_off(cd);
			}
		}
	}
...
static void pt_fingerprint_aoi_on(struct pt_core_data *cd)
{
	int retval;

	retval = 0;
	retval |= pt_pip_set_param_(cd, 0x31, cd->fingerprint_aoi_area.left, 2);
	retval |= pt_pip_set_param_(cd, 0x32, cd->fingerprint_aoi_area.right, 2);
	retval |= pt_pip_set_param_(cd, 0x33, cd->fingerprint_aoi_area.top, 2);
	retval |= pt_pip_set_param_(cd, 0x34, cd->fingerprint_aoi_area.bottom, 2);
	if (retval != 0) {
		pt_debug(cd->dev, DL_ERROR,
				"Failed to set fingerprint_aoi ON");
	}
}
static void pt_fingerprint_aoi_off(struct pt_core_data *cd)
{
	int retval;
	struct pt_mt_data *md = &cd->md;
	...
	if((md != NULL) && (md->si != NULL)) {
		retval = 0;
		retval |= pt_pip_set_param_(cd, 0x31, 0, 2);
		retval |= pt_pip_set_param_(cd, 0x32, md->si->sensing_conf_data.res_x - 1, 2);
		retval |= pt_pip_set_param_(cd, 0x33, 0, 2);
		retval |= pt_pip_set_param_(cd, 0x34, md->si->sensing_conf_data.res_y - 1, 2);
	}
	else {
		retval = -1;
	}
	if (retval != 0) {
		pt_debug(cd->dev, DL_ERROR,
				"Failed to set fingerprint_aoi OFF");
	}
}
static int pt_fingerprint_aoi_set_state(struct pt_core_data *cd, int on)
{
	int rc;
	...
	if(on){
		if(cd->fingerprint_aoi_req_state == 0){
			cd->fingerprint_aoi_req_state = 1;
			pt_debug(cd->dev, DL_DEBUG, "%s: [fingerprint_aoi]<ON>\n",
				__func__);

			if (cd->sleep_state == SS_SLEEP_OFF) {
				pt_fingerprint_aoi_on(cd);
			}
		}
	}
	else{
		if(cd->fingerprint_aoi_req_state != 0){
			cd->fingerprint_aoi_req_state = 0;
			pt_debug(cd->dev, DL_DEBUG, "%s: [fingerprint_aoi]<OFF>\n",
				__func__);

			if (cd->sleep_state == SS_SLEEP_OFF) {
				pt_fingerprint_aoi_off(cd);
			}
		}
	}
...
/* -----------------------------------*/
int pt_fingerprint_notify_set_enable(struct device *dev, int enable)
{
	struct pt_core_data *cd = dev_get_drvdata(dev);
	pt_fingerprint_notify_set_state(cd, enable);
	return 0;
}
int pt_fingerprint_notify_test(struct device *dev, int high)
{
	struct pt_core_data *cd = dev_get_drvdata(dev);
	pt_fingerprint_notify_set_test(cd, high);
	return 0;
}
int pt_fingerprint_notify_aoi_set(struct device *dev, struct pt_fingerprint_aoi_area *area)
{
	struct pt_core_data *cd = dev_get_drvdata(dev);
	int rc;
	...
	cd->fingerprint_aoi_area.left	= area->left;
	cd->fingerprint_aoi_area.top	= area->top;
	cd->fingerprint_aoi_area.right	= area->right;
	cd->fingerprint_aoi_area.bottom	= area->bottom;
...
int pt_fingerprint_notify_aoi_get(struct device *dev, struct pt_fingerprint_aoi_area *area)
{
	struct pt_core_data *cd = dev_get_drvdata(dev);
	int rc;
	...
	area->left		= cd->fingerprint_aoi_area.left;
	area->top		= cd->fingerprint_aoi_area.top;
	area->right		= cd->fingerprint_aoi_area.right;
	area->bottom	= cd->fingerprint_aoi_area.bottom;
...
int pt_fingerprint_notify_aoi_set_enable(struct device *dev, int enable)
{
	struct pt_core_data *cd = dev_get_drvdata(dev);
	pt_fingerprint_aoi_set_state(cd, enable);
	return 0;
}
int pt_fingerprint_notify_aoi_get_enable(struct device *dev)
{
	struct pt_core_data *cd = dev_get_drvdata(dev);
	return cd->fingerprint_aoi_req_state;
}
#endif /* PARADE_FINGERPRINT_NOTIFY_ENABLE */

#if defined(PARADE_GRIP_SUPPRESSION_ENABLE)
static void pt_set_grip_suppression_on(struct pt_core_data *cd)
{
	int retval;

	retval = 0;
	retval |= pt_pip_set_param_(cd, 0x0E, PARADE_PRM_GRIP_X_EDGE_LFT, 2);
	retval |= pt_pip_set_param_(cd, 0x0F, PARADE_PRM_GRIP_X_EDGE_RGT, 2);
	retval |= pt_pip_set_param_(cd, 0x10, PARADE_PRM_GRIP_X_EXC_LFT, 2);
	retval |= pt_pip_set_param_(cd, 0x11, PARADE_PRM_GRIP_X_EXC_RGT, 2);
	retval |= pt_pip_set_param_(cd, 0x12, PARADE_PRM_GRIP_Y_EDGE_UPR, 2);
	retval |= pt_pip_set_param_(cd, 0x13, PARADE_PRM_GRIP_Y_EDGE_LWR, 2);
	retval |= pt_pip_set_param_(cd, 0x14, PARADE_PRM_GRIP_Y_EXC_UPR, 2);
	retval |= pt_pip_set_param_(cd, 0x15, PARADE_PRM_GRIP_Y_EXC_LWR, 2);
	retval |= pt_pip_set_param_(cd, 0x16, 0x01, 1);
	retval |= pt_pip_set_param_(cd, 0x16, PARADE_PRM_GRIP_FIRST_EXC, 1);
	retval |= pt_pip_set_param_(cd, 0x17, PARADE_PRM_GRIP_EXC_EDGE_ORIGIN, 1);
	retval |= pt_pip_set_param_(cd, 0x18, 0x01, 1);
	if (retval != 0) {
		pt_debug(cd->dev, DL_ERROR,
				"Failed to set grip_suppression ON");
	}
}
static void pt_set_grip_suppression_off(struct pt_core_data *cd)
{
	int retval;

	retval = pt_pip_set_param_(cd, 0x18, 0x00, 1);
	if (retval != 0) {
		pt_debug(cd->dev, DL_ERROR,
				"Failed to set grip_suppression OFF");
	}
}
static void pt_set_grip_suppression_init(struct pt_core_data *cd)
{
	if(PARADE_PRM_GRIP_SUPPRESSION_ENABLE != 0) {
		pt_set_grip_suppression_on(cd);
	}else{
		pt_set_grip_suppression_off(cd);
	}
}
#endif /* PARADE_GRIP_SUPPRESSION_ENABLE */

#if defined(PARADE_CTRL_FOS_ENABLE_PARAM_ENABLE)
static void pt_ctrl_fos_enable_param_on(struct pt_core_data *cd)
{
	int retval;

	retval = 0;
	retval = pt_pip_set_param_(cd, 0x60, 0x01, 1);
	if (retval != 0) {
		pt_debug(cd->dev, DL_ERROR,
				"Failed to set fos_enable ON");
	}

	cd->fos_enable_state = 1;
}
static void pt_ctrl_fos_enable_param_off(struct pt_core_data *cd)
{
	int retval;

	retval = pt_pip_set_param_(cd, 0x60, 0x00, 1);
	if (retval != 0) {
		pt_debug(cd->dev, DL_ERROR,
				"Failed to set fos_enable OFF");
	}

	cd->fos_enable_state = 0;
}
static void pt_ctrl_fos_enable_param_enable(struct pt_core_data *cd)
{
	if(PARADE_PRM_CTRL_FOS_ENABLE_PARAM_ENABLE != 0) {
		pt_ctrl_fos_enable_param_on(cd);
		schedule_delayed_work(&cd->ctrl_fos_enable_param_delayed_work, msecs_to_jiffies(PARADE_PRM_CTRL_FOS_ENABLE_PARAM_DELAY_TIME_MS));
	}
}
static void pt_ctrl_fos_enable_param_disable(struct pt_core_data *cd)
{
	if(PARADE_PRM_CTRL_FOS_ENABLE_PARAM_ENABLE != 0) {
		cancel_delayed_work(&cd->ctrl_fos_enable_param_delayed_work);
		pt_ctrl_fos_enable_param_on(cd);
	}
}
static void pt_ctrl_fos_enable_param_work(struct work_struct *work)
{
	struct delayed_work *dw = container_of(work, struct delayed_work, work);
	struct pt_core_data *cd = container_of(dw, struct pt_core_data, ctrl_fos_enable_param_delayed_work);
	...
	if (cd->sleep_state == SS_SLEEP_OFF) {
		if((md != NULL) && (md->si != NULL)) {
			if (md->num_prv_rec == 0) {
#if defined(PARADE_SUPPORT_DEGRADE_FLG_ENABLE)
				if (time_after(jiffies, md->degrade_flg_info.initialize_baselines_end_time_jiffies
								+ msecs_to_jiffies(PARADE_PRM_CTRL_FOS_ENABLE_PARAM_INITIALIZE_BASELINES_WAIT_TIME_MS)) == true) {

					pt_ctrl_fos_enable_param_off(cd);
				}
				else {
					pt_debug(cd->dev, DL_INFO, "%s: Delayed due to initialize_baselines wait.(%dms)\n", __func__, PARADE_PRM_CTRL_FOS_ENABLE_PARAM_RETRY_TIME_MS);
					schedule_delayed_work(&cd->ctrl_fos_enable_param_delayed_work, msecs_to_jiffies(PARADE_PRM_CTRL_FOS_ENABLE_PARAM_RETRY_TIME_MS));
				}
#else
				pt_ctrl_fos_enable_param_off(cd);
#endif /* PARADE_SUPPORT_DEGRADE_FLG_ENABLE */
				...
				pt_debug(cd->dev, DL_INFO, "%s: Delayed due to touching finger.(%dms)\n", __func__, PARADE_PRM_CTRL_FOS_ENABLE_PARAM_RETRY_TIME_MS);
				schedule_delayed_work(&cd->ctrl_fos_enable_param_delayed_work, msecs_to_jiffies(PARADE_PRM_CTRL_FOS_ENABLE_PARAM_RETRY_TIME_MS));
			}
		}
	}
	else {
		pt_debug(cd->dev, DL_INFO, "%s: skip because suspended.\n", __func__);
	}
...
void pt_request_fos_enable_param_enable(struct pt_core_data *cd, int enable_time_ms)
{
	if(PARADE_PRM_CTRL_FOS_ENABLE_PARAM_ENABLE != 0) {
		cancel_delayed_work(&cd->ctrl_fos_enable_param_delayed_work);
		pt_ctrl_fos_enable_param_on(cd);
		schedule_delayed_work(&cd->ctrl_fos_enable_param_delayed_work, msecs_to_jiffies(enable_time_ms));
	}
}
#endif /* PARADE_CTRL_FOS_ENABLE_PARAM_ENABLE */
...
#if defined(PARADE_ASYNC_WAKE_ENABLE)
#ifdef CONFIG_DRM_PANEL
	case PT_DRV_DBG_DRM_NOTIFIER_ON:
	{
		struct drm_panel_notifier evdata;
		int blank;
#if defined(PARADE_ALWAYS_ON_DISPLAY_ENABLE)
		blank = DRM_PANEL_EVENT_DPMS_ON;
		evdata.data = (void*)&blank;
		cd->drm_notifier.notifier_call(&cd->drm_notifier, DRM_PANEL_EVENT_DPMS, (void*)&evdata);
#else
		blank = DRM_PANEL_BLANK_UNBLANK;
		evdata.data = (void*)&blank;
		cd->drm_notifier.notifier_call(&cd->drm_notifier, DRM_PANEL_EVENT_BLANK, (void*)&evdata);
#endif /* PARADE_ALWAYS_ON_DISPLAY_ENABLE */
		break;
	}
	case PT_DRV_DBG_DRM_NOTIFIER_OFF:
	{
		struct drm_panel_notifier evdata;
		int blank;
#if defined(PARADE_ALWAYS_ON_DISPLAY_ENABLE)
		...
		evdata.data = (void*)&blank;
		cd->drm_notifier.notifier_call(&cd->drm_notifier, DRM_PANEL_EARLY_EVENT_BLANK, (void*)&evdata);
#endif /* PARADE_ALWAYS_ON_DISPLAY_ENABLE */
		break;
	}
#endif
...
#if defined(PARADE_LPMODE_ENABLE)
static ssize_t pt_sysfs_lpmode_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	pt_debug(dev, DL_INFO, "%s: start. value=%d\n", __func__, input);

	pt_lpmode_set_state(cd, PARADE_LPMODE_TYPE_NON_CONTINUOUS, PARADE_LPMODE_REQ_COMMON, input);

	pt_debug(dev, DL_INFO, "%s: end. value=%d\n", __func__, input);
	return count;
}
static ssize_t pt_sysfs_lpmode_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct pt_core_data *cd = dev_get_drvdata(dev);
...
static ssize_t pt_sysfs_continuous_lpmode_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count)
{
	unsigned int input;
	struct pt_core_data *cd = dev_get_drvdata(dev);
	...
	pt_lpmode_set_state(cd, PARADE_LPMODE_TYPE_CONTINUOUS, PARADE_LPMODE_REQ_ECO, input);

	pt_debug(dev, DL_INFO, "%s: end. value=%d\n", __func__, input);
	return count;
}
static ssize_t pt_sysfs_continuous_lpmode_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct pt_core_data *cd = dev_get_drvdata(dev);

	return snprintf(buf, PAGE_SIZE, "0x%02X\n", cd->lpmode_continuous_req_state);
}
#endif /* PARADE_LPMODE_ENABLE */

#if defined(PARADE_GLOVE_ENABLE)
static ssize_t pt_sysfs_glove_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count)
{
	struct pt_core_data *cd = dev_get_drvdata(dev);
	unsigned long value;
	...
	rc = pt_set_glove_state(cd, value);
	if (rc < 0) {
		pt_debug(dev, DL_ERROR, "%s: error. rc=%d\n", __func__, rc);
		return -EINVAL;
	}
...
static ssize_t pt_sysfs_glove_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct pt_core_data *cd = dev_get_drvdata(dev);
	ssize_t ret;

	ret = snprintf(buf, PT_MAX_PRBUF_SIZE, "%d\n",
			cd->glove_enable_state);
	return ret;
}
#endif /* PARADE_GLOVE_ENABLE */

#if defined(PARADE_CTRL_FW_REPORT_RATE_ENABLE)
static ssize_t pt_sysfs_fw_report_rate_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count)
{
	struct pt_core_data *cd = dev_get_drvdata(dev);
	unsigned long value;
	...
	rc = pt_set_fw_report_rate_state(cd, value);
	if (rc < 0) {
		pt_debug(dev, DL_ERROR, "%s: error. rc=%d\n", __func__, rc);
		return -EINVAL;
	}
...
static ssize_t pt_sysfs_fw_report_rate_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct pt_core_data *cd = dev_get_drvdata(dev);
	ssize_t ret;
	...
	ret = snprintf(buf, PT_MAX_PRBUF_SIZE, "%d\n",
			cd->fw_report_rate_req_state);
	return ret;
}
#endif /* PARADE_CTRL_FW_REPORT_RATE_ENABLE */

#if defined(PARADE_LPWG_ENABLE)
static ssize_t pt_sysfs_lpwg_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count)
{
	struct pt_core_data *cd = dev_get_drvdata(dev);
	int rc = 0;
	...
	/* Maximmum input is two elements */
	length = _pt_ic_parse_input(dev, buf, count,
			input_data, ARRAY_SIZE(input_data));
	if (length != 2) {
		pt_debug(dev, DL_ERROR, "%s: Invalid value\n", __func__);
		return -EINVAL;
	...
	pt_debug(dev, DL_INFO, "%s: start. gesture=%d para=%d\n", __func__, para1, para2);

	if (para1 == 0) {
		gesture = PARADE_LPWG_GESTURE_SINGLE_TAP;
	}
	else if (para1 == 1) {
		gesture = PARADE_LPWG_GESTURE_DOUBLE_TAP;
	}
	else if (para1 == 2) {
		gesture = PARADE_LPWG_GESTURE_SINGLE_FINGER_LONG_TOUCH;
	}
	else if (para1 == 3) {
		gesture = PARADE_LPWG_GESTURE_2_FINGERS_LONG_TOUCH;
	}
	else {
		pt_debug(dev, DL_ERROR, "%s: Invalid para1\n", __func__);
		return -EINVAL;
	}

	rc = pt_lpwg_set_state(cd, gesture, (para2 == 0 ? 0 : 1));
	if (rc < 0) {
		pt_debug(dev, DL_ERROR, "%s: error. rc=%d\n", __func__, rc);
		return -EINVAL;
	}

	pt_debug(dev, DL_INFO, "%s: end. gesture=%d para=%d\n", __func__, para1, para2);
	return count;
}
static ssize_t pt_sysfs_lpwg_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct pt_core_data *cd = dev_get_drvdata(dev);
	ssize_t ret;
...
#ifdef PARADE_SEND_SHTERM_EVENT_ENABLE
static ssize_t pt_sysfs_battlog_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count)
{
	unsigned int input;
	...
	if(input == 1){
		info.event_num = SHBATTLOG_EVENT_TPS_CRC_ERROR;
		shterm_k_set_event(&info);
	}
	else if(input == 2){
		info.event_num = SHBATTLOG_EVENT_TPS_CRC_ERROR_MAX;
		shterm_k_set_event(&info);
	}
	else if(input == 3){
		info.event_num = SHBATTLOG_EVENT_TPS_CRC_ERROR_FIX;
		shterm_k_set_event(&info);
	}
...
#if defined(PARADE_CHARGER_ARMOR_ENABLE)
static ssize_t pt_sysfs_charger_armor_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count)
{
	struct pt_core_data *cd = dev_get_drvdata(dev);
	unsigned long value;
	...
	rc = pt_set_charger_armor_state(cd, value);
	if (rc < 0) {
		pt_debug(dev, DL_ERROR, "%s: error. rc=%d\n", __func__, rc);
		return -EINVAL;
	}
...
static ssize_t pt_sysfs_charger_armor_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct pt_core_data *cd = dev_get_drvdata(dev);
	ssize_t ret;
...
#endif /* TTDL_DIAGNOSTICS */
#if defined(PARADE_LPMODE_ENABLE)
	__ATTR(lpmode, (S_IRUGO | S_IWUSR | S_IWGRP), pt_sysfs_lpmode_show,
		pt_sysfs_lpmode_store),
	__ATTR(continuous_lpmode, (S_IRUGO | S_IWUSR | S_IWGRP), pt_sysfs_continuous_lpmode_show,
		pt_sysfs_continuous_lpmode_store),
#endif /* PARADE_LPMODE_ENABLE */
#if defined(PARADE_GLOVE_ENABLE)
	__ATTR(glove, (S_IRUGO | S_IWUSR | S_IWGRP), pt_sysfs_glove_show,
		pt_sysfs_glove_store),
#endif /* PARADE_GLOVE_ENABLE */
#if defined(PARADE_CTRL_FW_REPORT_RATE_ENABLE)
	__ATTR(fw_report_rate, (S_IRUGO | S_IWUSR | S_IWGRP), pt_sysfs_fw_report_rate_show,
		pt_sysfs_fw_report_rate_store),
#endif /* PARADE_CTRL_FW_REPORT_RATE_ENABLE */
#if defined(PARADE_LPWG_ENABLE)
	__ATTR(lpwg, (S_IRUGO | S_IWUSR | S_IWGRP), pt_sysfs_lpwg_show,
		pt_sysfs_lpwg_store),
#endif /* PARADE_LPWG_ENABLE */
#ifdef PARADE_SEND_SHTERM_EVENT_ENABLE
	__ATTR(battlog, (S_IWUSR | S_IWGRP), NULL,
		pt_sysfs_battlog_store),
#endif /* PARADE_SEND_SHTERM_EVENT_ENABLE */
#if defined(PARADE_CHARGER_ARMOR_ENABLE)
	__ATTR(charger_armor, (S_IRUGO | S_IWUSR | S_IWGRP), pt_sysfs_charger_armor_show,
		pt_sysfs_charger_armor_store),
#endif /* PARADE_CHARGER_ARMOR_ENABLE */
};
...
#if defined(PARADE_TPIN_CHECK_ENABLE)
static int pt_tpin_enable_check(struct pt_core_platform_data *pdata, struct device *dev)
{
	#ifdef CONFIG_OF
		int val;

		if(pdata->test_mode_gpio == -1){
			pt_debug(dev, DL_ERROR,
					"test_mode_gpio not set.\n");
			return 0;
		}
		else if (IS_ERR_OR_NULL(pdata->pinctrl)) {
			pt_debug(dev, DL_ERROR,
					"pinctrl not set.\n");
			return 0;
		}
		else {
			gpio_request(pdata->test_mode_gpio, "pt_tpin");
			pinctrl_select_state(pdata->pinctrl, pdata->tpin_state_active);
			mb();
			udelay(10);
			val = gpio_get_value(pdata->test_mode_gpio);
			pinctrl_select_state(pdata->pinctrl, pdata->tpin_state_suspend);
			gpio_free(pdata->test_mode_gpio);

			if(!val) {
				pt_debug(dev, DL_ERROR,
						"Upper unit does not exist.\n");
				return -1;
			}
		}
	#endif /* CONFIG_OF */
...
#endif /* PARADE_TPIN_CHECK_ENABLE */

#ifdef CONFIG_DRM_PANEL
int pt_drm_notifier_cb(struct notifier_block *self, unsigned long event, void *data)
{
	struct drm_panel_notifier *evdata = data;
	int blank;
	struct pt_core_data *cd =
			container_of(self, struct pt_core_data, drm_notifier);
#if defined(PARADE_ASYNC_WAKE_ENABLE)
	int t;
#endif /* PARADE_ASYNC_WAKE_ENABLE */
	...
	if (evdata && evdata->data) {
		if ((event == DRM_PANEL_EVENT_BLANK) || (event == DRM_PANEL_EARLY_EVENT_BLANK)) {
...
#if defined(PARADE_ASYNC_WAKE_ENABLE)
				if(cd->async_wake_flg == false) {
					cd->async_wake_flg = true;
					schedule_work(&cd->async_wake_work);
				}
#else
				pt_core_wake(cd);
#endif /* PARADE_ASYNC_WAKE_ENABLE */
			...
			pt_debug(cd->dev, DL_WARN, "%s: end. event=%d blank=%d\n",
				__func__, event, *(int *)evdata->data);
		}
	}
	return 0;
}
static void pt_panel_notifier_register_work(struct work_struct *work)
{
	struct pt_core_data *cd =
			container_of(work, struct pt_core_data,
	...
	for (retry = 0; retry <= PARADE_PANEL_NOTIFIER_REGISTER_RETRY_MAX; retry++) {
		if (cd->panel_notifier_register_work_flg == false) {
			break;
		}

		count = of_count_phandle_with_args(cd->dev->of_node, "panel", NULL);
		if (count > 0) {
			for (i = 0; i < count; i++) {
				node = of_parse_phandle(cd->dev->of_node, "panel", i);
		...
		if (active_panel) {
			cd->drm_notifier.notifier_call = pt_drm_notifier_cb;

			rc = drm_panel_notifier_register(active_panel,
					&cd->drm_notifier);
			if (rc < 0) {
				if(retry < PARADE_PANEL_NOTIFIER_REGISTER_RETRY_MAX) {
					pt_debug(cd->dev, DL_ERROR,
							"%s: Failed to register panel notifier client. retry(%d/%d)\n",
							__func__, retry, PARADE_PANEL_NOTIFIER_REGISTER_RETRY_MAX);

					msleep(PARADE_PANEL_NOTIFIER_REGISTER_RETRY_INTERVAL_MS);
				}
				else {
					pt_debug(cd->dev, DL_ERROR,
...
#if defined(PARADE_ASYNC_WAKE_ENABLE)
	init_waitqueue_head(&cd->async_wake_wait_q);
#endif /* PARADE_ASYNC_WAKE_ENABLE */
...
#if defined(PARADE_ASYNC_WAKE_ENABLE)
	INIT_WORK(&cd->async_wake_work, pt_async_wake_work);
#endif /* PARADE_ASYNC_WAKE_ENABLE */
...
#if defined(PARADE_TPIN_CHECK_ENABLE)
	rc = pt_tpin_enable_check(cd->cpdata, cd->dev);
	if (rc) {
		rc = -ENODEV;
		goto error_detect;
	}
...
#if defined(PARADE_LPWG_ENABLE)
	cd->lpwg_system_suspended = false;
	mutex_init(&cd->lpwg_wake_lock_on_mutex);
	cd->lpwg_wake_lock_on = false;
	rc = wake_lock_init(&cd->lpwg_wake_lock, cd->dev, "pt_lpwg_wake_lock");
	if (rc) {
		pt_debug(dev, DL_ERROR, "%s: Error, could not setup lpwg_wake_lock\n", __func__);
		goto error_setup_lpwg_wake_lock;
	}
	INIT_DELAYED_WORK(&cd->lpwg_wake_lock_timer, pt_lpwg_wake_lock_timer);
#endif /* PARADE_LPWG_ENABLE */
#if defined(PARADE_CTRL_FOS_ENABLE_PARAM_ENABLE)
	INIT_DELAYED_WORK(&cd->ctrl_fos_enable_param_delayed_work, pt_ctrl_fos_enable_param_work);
#endif /* PARADE_CTRL_FOS_ENABLE_PARAM_ENABLE */
...
#endif
#ifdef PARADE_USE_CLASS_IF
	shtps_class_if_init(dev);
#endif
...
#ifdef CONFIG_DRM_PANEL
	cd->panel_notifier_register_work_flg = true;
	INIT_WORK(&cd->panel_notifier_register_work, pt_panel_notifier_register_work);
	schedule_work(&cd->panel_notifier_register_work);
#endif
...
#if defined(PARADE_ASYNC_WAKE_ENABLE)
	cancel_work_sync(&cd->async_wake_work);
#endif /* PARADE_ASYNC_WAKE_ENABLE */
	cancel_work_sync(&cd->enum_work);
	pt_stop_wd_timer(cd);
	pt_free_si_ptrs(cd);
...
error_setup_irq:
#if defined(PARADE_LPWG_ENABLE)
	wake_lock_destroy(&cd->lpwg_wake_lock);
error_setup_lpwg_wake_lock:
#endif /* PARADE_LPWG_ENABLE */
error_detect:
#ifdef PT_PTSBC_SUPPORT
...
int pt_release(struct pt_core_data *cd)
{
	...
	pt_debug(dev, DL_WARN, "%s: start\n",
		__func__);

#ifdef CONFIG_DRM_PANEL
	cd->panel_notifier_register_work_flg = false;
	cancel_work_sync(&cd->panel_notifier_register_work);
	if (pt_active_panel) {
		drm_panel_notifier_unregister(pt_active_panel, &cd->drm_notifier);
	}
#endif
#ifdef PARADE_USE_CLASS_IF
	shtps_class_if_exit();
