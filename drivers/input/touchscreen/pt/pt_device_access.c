...
#ifdef PARADE_SYSFS_CALIBRATE
static ssize_t pt_calibrate_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct pt_device_access_data *dad
		= pt_get_device_access_data(dev);
	...
	rc = _pt_calibrate_idacs_cmd(dev, dad->calibrate_sensing_mode,
			&dad->ic_buf[0]);
	if (rc < 0) {
		pt_debug(dev, DL_ERROR, "%s: Error on calibrate idacs r=%d\n",
				__func__, rc);
		goto resume_scan;
	}
	...
	if (dad->calibrate_initialize_baselines) {
		...
		rc = _pt_initialize_baselines_cmd(dev, PT_IB_SM_MUTCAP |
				...
				&dad->ic_buf[length]);
		if (rc < 0) {
			pt_debug(dev, DL_ERROR, "%s: Error on initialize baselines r=%d\n",
					__func__, rc);
			goto resume_scan;
		}
...
put_pm_runtime:
	...
	index = prepare_print_buffer(status, dad->ic_buf, length,
		buf, PAGE_SIZE, PT_PR_FORMAT_DEFAULT);
...
static ssize_t pt_calibrate_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t size)
{
	struct pt_device_access_data *dad
		= pt_get_device_access_data(dev);
	...
	length = cmd->parse_sysfs_input(dad->dev, buf, size,
			input_data, ARRAY_SIZE(input_data));
	if (length != 2) {
		pt_debug(dad->dev, DL_ERROR, "%s: Malformed input\n", __func__);
		rc = -EINVAL;
		goto exit_unlock;
	}
...
static DEVICE_ATTR(calibrate, S_IRUSR | S_IRGRP | S_IWUSR | S_IWGRP, 
	pt_calibrate_show, pt_calibrate_store);

static ssize_t pt_panel_scan_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct pt_device_access_data *dad
		= pt_get_device_access_data(dev);
...
put_pm_runtime:
	...
	if(dad->panel_scan_size > (PAGE_SIZE - 1)) {
		return_length = (PAGE_SIZE - 1);
	}
	else {
		return_length = dad->panel_scan_size;
	}
	memcpy(buf, buf_out, return_length);
	buf[return_length] = '\0';

	pt_debug(dev, DL_DEBUG, "%s: Sent %d/%d bytes to user space\n",
		__func__, return_length, print_idx);
...
static ssize_t pt_panel_scan_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t size)
{
	struct pt_device_access_data *dad
		= pt_get_device_access_data(dev);
...
static DEVICE_ATTR(panel_scan, S_IRUSR | S_IRGRP | S_IWUSR | S_IWGRP, pt_panel_scan_show, pt_panel_scan_store);

static ssize_t pt_cm_panel_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct pt_device_access_data *dad
		= pt_get_device_access_data(dev);
	...
	index = pt_run_and_get_selftest_result(
		dev, PT_CORE_CMD_PROTECTED,
		buf, PAGE_SIZE,
		PT_ST_ID_CM_PANEL, PIP_CMD_MAX_LENGTH,
...
static DEVICE_ATTR(cm_panel, S_IRUSR | S_IRGRP,
	pt_cm_panel_show, NULL);

static ssize_t pt_cp_panel_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct pt_device_access_data *dad
		= pt_get_device_access_data(dev);
...
static DEVICE_ATTR(cp_panel, S_IRUSR | S_IRGRP,
	pt_cp_panel_show, NULL);
#endif /* PARADE_SYSFS_CALIBRATE */
...
static int pt_setup_sysfs(struct device *dev)
{
	...
	if (rc) {
		pt_debug(dev, DL_ERROR, "%s: Error, could not create calibrate\n",
				__func__);
		goto unregister_cmcp_threshold_data;
	}
	rc = device_create_file(dev, &dev_attr_panel_scan);
	if (rc) {
		pt_debug(dev, DL_ERROR, "%s: Error, could not create panel_scan\n",
				__func__);
		goto unregister_calibrate;
	}
	rc = device_create_file(dev, &dev_attr_cm_panel);
	if (rc) {
		pt_debug(dev, DL_ERROR, "%s: Error, could not create cm_panel\n",
				__func__);
		goto unregister_panel_scan;
	}
	rc = device_create_file(dev, &dev_attr_cp_panel);
	if (rc) {
		pt_debug(dev, DL_ERROR, "%s: Error, could not create cp_panel\n",
				__func__);
		goto unregister_cm_panel;
	}
#endif /* PARADE_SYSFS_CALIBRATE */
...
#ifdef PARADE_SYSFS_CALIBRATE
unregister_cm_panel:
	device_remove_file(dev, &dev_attr_cm_panel);
unregister_panel_scan:
	device_remove_file(dev, &dev_attr_panel_scan);
unregister_calibrate:
	device_remove_file(dev, &dev_attr_calibrate);
unregister_cmcp_threshold_data:
#ifdef SUPPORT_GKI_BUILD
	sysfs_remove_bin_file(&dev->kobj, &bin_attr_cmcp_threshold_data);
#else
...
static int pt_setup_debugfs(struct device *dev)
{
	struct pt_device_access_data *dad
		= pt_get_device_access_data(dev);
	int rc = 0;
	...
	if (IS_ERR_OR_NULL(dad->base_dentry)) {
		pt_debug(dev, DL_ERROR,
			"%s: Error, could not create debugfs base directory\n",
			__func__);
		goto exit;
	}
	...
	dad->debugfs_nodes_created = true;
	return rc;
...
static void pt_device_access_release(struct device *dev, void *data)
{
	...
	if (dad->sysfs_nodes_created) {
		...
		device_remove_file(dev, &dev_attr_panel_scan);
		device_remove_file(dev, &dev_attr_cm_panel);
		device_remove_file(dev, &dev_attr_cp_panel);
#endif /* PARADE_SYSFS_CALIBRATE */
		kfree(dad->cmcp_threshold_data);
	} else {
		cmd->unsubscribe_attention(dev, PT_ATTEN_STARTUP,
