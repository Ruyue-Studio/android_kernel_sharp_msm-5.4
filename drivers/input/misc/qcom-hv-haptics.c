...
#define HAP_VMAX_MIN_REG        (VMAX_STEP_MV / VMAX_STEP_MV)
#define HAP_VMAX_MAX_REG        (MAX_VMAX_MV / VMAX_STEP_MV)
...
#ifdef CONFIG_INPUT_QTI_SHARP_HAPTICS
#ifdef CONFIG_SHARP_BOOT
int              shdiag_vib_param[9];
extern int              sh_boot_mode;

static int __init get_shdiag_vib_param(char *str)
{
	get_option(&str, &shdiag_vib_param[0]);

	return 1;
}

__setup("androidboot.vibinfo=", get_shdiag_vib_param);

static int qti_haptics_smem_config(struct haptics_chip *chip)
{
	int rc = 0;
	int i;
	u8 param[4];
	...
	if (!param[0] && !param[1] && !param[2] && !param[3]) {
		...
		if (qti_hap_set_smem_param) {
			rc = haptics_write(chip, chip->cfg_addr_base, HAP_CFG_AUTORES_CFG_REG, &chip->auto_res_reg, 1);
			if (rc) {
				dev_err(chip->dev, "[qpnp_hap]QPNP_HAP_LRA_AUTO_RES_REG org_param write err rc:%d\n", rc);
				return rc;
			}
			rc = haptics_write(chip, chip->cfg_addr_base, HAP_CFG_VMAX_REG, &chip->vmax_reg, 1);
			if (rc){
		...
		if ((param[0] == 0) || (param[0] == HAP_LRA_AUTO_RES_PARAM)) {
			rc = haptics_write(chip, chip->cfg_addr_base, HAP_CFG_AUTORES_CFG_REG, &param[0], 1);
			if (rc) {
				dev_err(chip->dev, "[qpnp_hap]QPNP_HAP_LRA_AUTO_RES_REG smem_param write err rc:%d\n", rc);
				return rc;
			}
		} else {
			dev_err(chip->dev, "[qpnp_hap]QPNP_HAP_LRA_AUTO_RES_REG smem_param write param err param:0x%02x\n", rc);
		}
		if ((param[1] >= HAP_VMAX_MIN_REG) && (param[1] <= HAP_VMAX_MAX_REG)) {
			rc = haptics_write(chip, chip->cfg_addr_base, HAP_CFG_VMAX_REG, &param[1], 1);
			if (rc) {
			...
			s16 __user *data, u32 length, s16 magnitude)
{
	...
	play->brake = NULL;
	play->vmax_mv = chip->custom_effect->vmax_mv;
	/* Toggle HAPTICS_EN for a clear start point of FIFO playing */
	rc = haptics_toggle_module_enable(chip);
	if (rc < 0)
		goto cleanup;
			...
			s16 __user *data, u32 length, s16 magnitude)
{
	...
	play->vmax_mv = chip->effects[i].vmax_mv;
...
#ifdef CONFIG_INPUT_QTI_SHARP_HAPTICS
#ifdef CONFIG_SHARP_BOOT
	if ((sh_boot_mode == SH_BOOT_D) || (sh_boot_mode == SH_BOOT_F_F)) {
							...
							u32 t_lra_us)
{
	...
	shval = 0x03;
        return haptics_write(chip, chip->cfg_addr_base,
                               HAP_CFG_TLRA_OL_HIGH_REG, &shval, 1);
#else
...
#ifdef CONFIG_INPUT_QTI_SHARP_HAPTICS
	chip->auto_res_reg = chip->effects->auto_res_disable;
	chip->vmax_reg     = (config->vmax_mv / VMAX_STEP_MV);
#endif /* CONFIG_INPUT_QTI_SHARP_HAPTICS */
...
static int haptics_probe(struct platform_device *pdev)
{
	...
	if (rc < 0) {
       ...
       shval = 0x65;
       rc = haptics_write(chip, chip->cfg_addr_base,
                               HAP_CFG_TLRA_OL_LOW_REG, &shval, 1);
	if (rc < 0)
		return rc;
#endif /* CONFIG_INPUT_QTI_SHARP_HAPTICS */
