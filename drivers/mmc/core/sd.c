...
unsigned mmc_sd_get_max_clock(struct mmc_card *card)
{
	...
	if (mmc_card_uhs(card)){
		if (card->sd_bus_speed == UHS_SDR104_BUS_SPEED)
			...
			strncpy(mmc_sd_clk_mode, "S050", sizeof(mmc_sd_clk_mode));
		else if (card->sd_bus_speed == UHS_SDR25_BUS_SPEED)
			strncpy(mmc_sd_clk_mode, "S025", sizeof(mmc_sd_clk_mode));
		else if (card->sd_bus_speed == UHS_SDR12_BUS_SPEED)
			strncpy(mmc_sd_clk_mode, "S012", sizeof(mmc_sd_clk_mode));
	}else if (mmc_card_hs(card)){
