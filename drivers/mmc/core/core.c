...
MODULE_PARM_DESC(sh_time_debug_mask, "MMC/SD cards tests param2");
static int sh_mmc_clk_debug_mask = 0;
module_param_named(sh_clk_debug_mask, sh_mmc_clk_debug_mask, int, S_IRUGO | S_IWUSR | S_IWGRP);
MODULE_PARM_DESC(sh_clk_debug_mask, "MMC/SD cards tests param3");
...
void mmc_request_done(struct mmc_host *host, struct mmc_request *mrq)
{
	...
	if (!err || !cmd->retries || mmc_card_removed(host->card)) {
		...
		if (sh_mmc_debug_mask && card &&
			...
			pr_info("mmc-shlog : %s: req done (CMD%u): %d: %08x %08x %08x %08x\n",
				mmc_hostname(host), cmd->opcode, err,
				cmd->resp[0], cmd->resp[1],
				cmd->resp[2], cmd->resp[3]);
		...
		if (sh_mmc_time_debug_mask && card &&
			...
			if (mrq->data)
				pr_info("mmc-shlog : (done) (CMD%u),%d,%d,%ld\n",
					mrq->cmd->opcode,
		...
		if (mrq->data) {
			...
			if (sh_mmc_debug_mask && card &&
				((card)->type == ((sh_mmc_debug_mask&0xF)-1)) &&
				((sh_mmc_debug_mask&0xF0)==0x10))
				pr_info("mmc-shlog : %s:     %d bytes transferred: %d\n",
					mmc_hostname(host),
					mrq->data->bytes_xfered, mrq->data->error);
#endif /* CONFIG_SHARP_MMC && CONFIG_ANDROID_ENGINEERING */
		}
		...
		if (mrq->stop) {
			...
			if (sh_mmc_debug_mask && card &&
				((card)->type == ((sh_mmc_debug_mask&0xF)-1)) &&
				((sh_mmc_debug_mask&0xF0)==0x10))
				pr_info("mmc-shlog : %s: stop (CMD%u): %d: %08x %08x %08x %08x\n",
					mmc_hostname(host), mrq->stop->opcode,
					mrq->stop->error,
					mrq->stop->resp[0], mrq->stop->resp[1],
					mrq->stop->resp[2], mrq->stop->resp[3]);
			     ...
			     bool cqe)
{
	...
	if (mrq->sbc) {
		...
		if (sh_mmc_debug_mask && card &&
			((card)->type == ((sh_mmc_debug_mask&0xF)-1)) &&
			((sh_mmc_debug_mask&0xF0)==0x10))
			pr_info("mmc-shlog :<%s: starting (CMD%u) arg %08x flags %08x>\n",
				 mmc_hostname(host), mrq->sbc->opcode,
				 mrq->sbc->arg, mrq->sbc->flags);
#endif /* CONFIG_SHARP_MMC && CONFIG_ANDROID_ENGINEERING */
	}
	...
	if (mrq->stop) {
		...
		if (sh_mmc_debug_mask && card &&
			((card)->type == ((sh_mmc_debug_mask&0xF)-1)) &&
			((sh_mmc_debug_mask&0xF0)==0x10))
			pr_info("mmc-shlog : %s:   stop (CMD%u) arg %08x flags %08x\n",
				 mmc_hostname(host), mrq->stop->opcode,
				 mrq->stop->arg, mrq->stop->flags);
#endif /* CONFIG_SHARP_MMC && CONFIG_ANDROID_ENGINEERING */
	}
...
void mmc_wait_for_req_done(struct mmc_host *host, struct mmc_request *mrq)
{
	...
	while (1) {
		...
		if (sh_mmc_debug_mask && card &&
			((card)->type == ((sh_mmc_debug_mask&0xF)-1)) &&
			((sh_mmc_debug_mask&0xF0)==0x10))
			pr_info("mmc-shlog : %s: req failed (CMD%u): %d, retrying...%d\n",
...
static inline void mmc_set_ios(struct mmc_host *host)
{
	...
	if (sh_mmc_clk_debug_mask)
		pr_info("mmc-shlog : %s: clock %uHz busmode %u powermode %u cs %u Vdd %u "
			"width %u timing %u\n",
			 mmc_hostname(host), ios->clock, ios->bus_mode,
			ios->power_mode, ios->chip_select, ios->vdd,
			1 << ios->bus_width, ios->timing);
