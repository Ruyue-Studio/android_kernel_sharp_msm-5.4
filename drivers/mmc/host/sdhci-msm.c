...
static int sdhci_msm_probe(struct platform_device *pdev)
{
	...
	{
		pr_info("mmc-shlog : %s: Qualcomm MSM SDHC-core device irq=%d\n",
				mmc_hostname(host->mmc), host->irq);
		pr_info("mmc-shlog : %s: SDHCI version: 0x%.8x flags: 0x%.8x\n",
				mmc_hostname(host->mmc), host->version, host->flags);
		pr_info("mmc-shlog : %s: host caps: 0x%.8x\n",
				mmc_hostname(host->mmc), host->caps);
		pr_info("mmc-shlog : %s: quirks: 0x%.8x quirks2: 0x%.8x\n",
				mmc_hostname(host->mmc), host->quirks, host->quirks2);
		pr_info("mmc-shlog : %s: mmc caps: 0x%.8x caps2:0x%.8x\n",
				mmc_hostname(host->mmc), msm_host->mmc->caps,
				msm_host->mmc->caps2);
		pr_info("mmc-shlog : %s: 8 bit data mode %s\n", mmc_hostname(host->mmc),
			(msm_host->mmc->caps & MMC_CAP_8_BIT_DATA ?
			 "enabled" : "disabled"));
		pr_info("mmc-shlog : %s: 4 bit data mode %s\n", mmc_hostname(host->mmc),
			(msm_host->mmc->caps & MMC_CAP_4_BIT_DATA ?
			 "enabled" : "disabled"));
		pr_info("mmc-shlog : %s: polling status mode %s\n", mmc_hostname(host->mmc),
			(msm_host->mmc->caps & MMC_CAP_NEEDS_POLL ?
			 "enabled" : "disabled"));
		pr_info("mmc-shlog : %s: MMC clock %u -> %u Hz\n",
		       mmc_hostname(host->mmc), sdhci_msm_get_min_clock(host),
			sdhci_msm_get_max_clock(host));

		pr_info("mmc-shlog : %s: HS200 1.8V mode %s\n", mmc_hostname(host->mmc),
			(msm_host->mmc->caps2 & MMC_CAP2_HS200_1_8V_SDR ?
			 "enabled" : "disabled"));
		pr_info("mmc-shlog : %s: HS200 1.2V mode %s\n", mmc_hostname(host->mmc),
			(msm_host->mmc->caps2 & MMC_CAP2_HS200_1_2V_SDR ?
			 "enabled" : "disabled"));
		pr_info("mmc-shlog : %s: DDR 1.8V mode %s\n", mmc_hostname(host->mmc),
			(msm_host->mmc->caps & MMC_CAP_1_8V_DDR ?
			 "enabled" : "disabled"));
		pr_info("mmc-shlog : %s: DDR 1.2V mode %s\n", mmc_hostname(host->mmc),
			(msm_host->mmc->caps & MMC_CAP_1_2V_DDR ?
			 "enabled" : "disabled"));
		pr_info("mmc-shlog : %s: HS400 1.8V mode %s\n", mmc_hostname(host->mmc),
			(msm_host->mmc->caps2 & MMC_CAP2_HS400_1_8V ?
			 "enabled" : "disabled"));
		pr_info("mmc-shlog : %s: HS400 1.2V mode %s\n", mmc_hostname(host->mmc),
			(msm_host->mmc->caps2 & MMC_CAP2_HS400_1_2V ?
			 "enabled" : "disabled"));
		pr_info("mmc-shlog : %s: UHS(SDR104) mode %s\n", mmc_hostname(host->mmc),
			(msm_host->mmc->caps & MMC_CAP_UHS_SDR104 ?
			 "enabled" : "disabled"));
		pr_info("mmc-shlog : %s: UHS(DDR50) mode %s\n", mmc_hostname(host->mmc),
			(msm_host->mmc->caps & MMC_CAP_UHS_DDR50 ?
				 "enabled" : "disabled"));

		if ((msm_host->vreg_data != NULL) && (msm_host->vreg_data->vdd_data != NULL))
			pr_info("mmc-shlog : %s: %s, always_on=%d, lpm_sup=%d, vol=[%d %d]uV,"
					" curr[%d %d]uA\n",
				mmc_hostname(host->mmc),
				msm_host->vreg_data->vdd_data->name,
				msm_host->vreg_data->vdd_data->is_always_on,
				msm_host->vreg_data->vdd_data->lpm_sup,
				msm_host->vreg_data->vdd_data->low_vol_level,
				msm_host->vreg_data->vdd_data->high_vol_level,
				msm_host->vreg_data->vdd_data->lpm_uA,
				msm_host->vreg_data->vdd_data->hpm_uA);
		else
			pr_info( "mmc-shlog : %s: <no vdd data>\n", mmc_hostname(host->mmc));

		if ((msm_host->vreg_data != NULL) && (msm_host->vreg_data->vdd_io_data != NULL))
			pr_info("mmc-shlog : %s: %s, always_on=%d, lpm_sup=%d, vol=[%d %d]uV,"
					" curr[%d %d]uA\n",
				mmc_hostname(host->mmc),
				msm_host->vreg_data->vdd_io_data->name,
				msm_host->vreg_data->vdd_io_data->is_always_on,
				msm_host->vreg_data->vdd_io_data->lpm_sup,
				msm_host->vreg_data->vdd_io_data->low_vol_level,
				msm_host->vreg_data->vdd_io_data->high_vol_level,
				msm_host->vreg_data->vdd_io_data->lpm_uA,
				msm_host->vreg_data->vdd_io_data->hpm_uA );
		else
			pr_info( "mmc-shlog : %s: <no vdd-io data>\n", mmc_hostname(host->mmc));
	}
