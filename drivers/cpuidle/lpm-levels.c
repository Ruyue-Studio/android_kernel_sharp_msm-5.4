...
static uint64_t idle_total_time = 0;
static uint64_t idle_enter_time = 0;
...
#ifdef CONFIG_SHARP_PNP_SLEEP_SLEEPLOG
	if (dev->cpu == 0 && idx == (cpu->nlevels - 1))
		idle_enter_time = start_time;
#endif /* CONFIG_SHARP_PNP_SLEEP_SLEEPLOG */
...
#ifdef CONFIG_SHARP_PNP_SLEEP_SLEEPLOG
	if (success && dev->cpu == 0 && idx == (cpu->nlevels - 1) && (end_time - idle_enter_time) > 0)
		idle_total_time += (end_time - idle_enter_time);
#endif /* CONFIG_SHARP_PNP_SLEEP_SLEEPLOG */
