...
#define SHARP_SPI_AUTO_SUSPEND_DELAY	(50)
...
static int spi_geni_prepare_transfer_hardware(struct spi_master *spi)
{
	 ...
	 */
	if (mas->is_le_vm) {
		SH_SPILOG_DEBUG(SH_LOG_TRACE_H, mas->dev, "Not required for LE\n");
		return 0;
	};
				...
				struct spi_master *spi)
{
	...
	mb();
	SH_SPILOG_DEBUG(SH_LOG_TRACE_M, mas->dev, "end (ret:%d)\n", ret);
	return ret;
}
...
static int spi_geni_probe(struct platform_device *pdev)
{
	...
	if (!geni_mas->dis_autosuspend) {
		...
		if (of_property_read_u32(pdev->dev.of_node, "spi-autosuspend-delay-time-sh",
					&autosuspend_delay)) {
			autosuspend_delay = SHARP_SPI_AUTO_SUSPEND_DELAY;
		}
...
static int spi_geni_runtime_suspend(struct device *dev)
{
	...
	if (geni_mas->is_le_vm) {
		SH_SPILOG_DEBUG(SH_LOG_PM, dev, "Not required for LE\n");
