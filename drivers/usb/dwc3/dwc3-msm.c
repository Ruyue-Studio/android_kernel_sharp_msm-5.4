					...
					bool enable_wakeup)
{
	...
	can_suspend_ssphy = (dwc->maximum_speed >= USB_SPEED_SUPER || dwc->max_hw_supp_speed >= USB_SPEED_SUPER) &&
...
void dwc3_otg_limitlock(void)
{
	...
	unsigned int perflock_freq = UINT_MAX;
	for_each_possible_cpu(cpu_id) {
		if (cpu_id <= 3)
			perflock_freq = 940800;
		if (cpu_id >=  4)
			perflock_freq = 940800;
		req = &per_cpu(qos_req_max, cpu_id);
		freq_qos_update_request(req, perflock_freq);
		}
}
void dwc3_otg_limitunlock(void)
{
	...
	for_each_possible_cpu(cpu_id) {
		req = &per_cpu(qos_req_max, cpu_id);
		freq_qos_update_request(req, UINT_MAX);
		}
}
...
static void dwc3_otg_limitlock_init()
{
	...
	for_each_possible_cpu(cpu_id) {
		...
		if (policy) {
			freq_qos_add_request(&policy->constraints, req, FREQ_QOS_MAX, policy->max);
		}
	}
}
